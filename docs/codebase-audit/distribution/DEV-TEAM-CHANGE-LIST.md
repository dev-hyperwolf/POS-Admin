...
| 11 | Surgical super-admin fixes (while the console is built) | The existing screens keep being used | `distributionManagement/index.js:72-78` use `<StatusText/>`; `addDistribution/index.js:134` delete the stray `return`; re-enable the available-qty guard in add-product-refill validation; un-comment "Aging (Days)" at `agingProducts/index.js:121-128`; rename "Yes, Distribute" to "Freeze" in `freezeDistribution.jsx:138` | each visible on the screen |
| 12 | Structured warnings in build/refill responses | The UI shows a green toast for a partial run | add `{skipped:[{subRegionId, reason}], warnings:[]}` beside the existing fields (additive) | a run that skips a subregion returns it |
| 13 | Fix `totalInventoryValue` | qty × dollar total (`closure-overview-controller.js:993`) | sum of qty × unit value | the closure detail equals a hand calculation |
