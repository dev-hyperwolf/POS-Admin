...
| 1.9 | **Purchase limits are effectively unenforced (compliance).** stilo-backend's limit feature buckets by is-cannabis instead of is-concentrate, overwrites instead of sums, checks one store in a UTC day, fails open when weight is missing, the medical row never matches ("Medicinal" vs "MedicinalUser") so medical customers are unlimited, under-21 members are auto-medical, and `createOrder` never calls the check. hyperwolf-backend has no limit code and relies on Blaze's "User Cannabis Limit" toggle (unconfirmed); sign-up marks every user's recommendation as verified. Until the shared limits engine ships (`docs/PURCHASE-LIMITS-PLAN-2026-09-17.md`): confirm the Blaze toggle is ON for every store, fix the medical string mismatch, sum instead of overwrite, fail closed on missing weight, and call the check from `createOrder` | `stilo-backend/common/utils.js:2950-3049,2823-2834,2999,3008-3016`; `stilo-backend/.../AddLimit.jsx:119`; `stilo-backend/controllers/member/member-controllers.js:77`; `hyperwolf-backend/controllers/user-auth-controllers.js:177-178`; `hyperwolf-backend/controllers/personaController.js:284-320` | a cart over 28.5 g flower or 8 g concentrate (adult-use) is refused at checkout in every channel; a medical customer's limit row matches | 0.5 |
