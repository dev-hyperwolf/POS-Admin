...
window.LReassign = function LReassign({ order, drivers, onPick, onClose, anchor }) {
  ...
  return <div data-reassign style={{ position: 'fixed', left, top, width: W, background: P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r12, boxShadow: P.shadowLg, zIndex: 300, overflow: 'hidden' }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 13px', borderBottom: `1px solid ${P.hairline}` }}>
      ...
      <span style={{ fontSize: 11.5, color: P.inkMute, fontFamily: P.fontMono }}>{L.regionLabel(order.region)}</span>
      ...
      {cands.map((c) => { const dis = c.current || !c.stock;
        return <button key={c.d.id} onClick={() => !dis && onPick(c.d.name)} disabled={dis} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '9px 13px', background: 'transparent', border: 'none', borderTop: `1px solid ${P.hairline}`, cursor: dis ? 'default' : 'pointer', textAlign: 'left', fontFamily: P.fontSans, opacity: dis ? .5 : 1 }}
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ fontSize: 12.5, fontWeight: 600, color: P.ink, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.d.name}</span>{c.current && <span style={{ fontSize: 10, fontWeight: 700, color: P.inkMute }}>CURRENT</span>}{c.idle && <Pill kind="warn" style={{ fontSize: 10, padding: '1px 6px' }}>idle</Pill>}</div>
            {c.stock ? <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 2, fontSize: 11.5, color: P.inkDim, fontFamily: P.fontMono }}><RegionTag code={c.d.region} size="sm" />{c.d.load} stops</div>
          ...
          <div style={{ textAlign: 'right', flex: '0 0 auto' }}>
            <div style={{ fontSize: 13.5, fontWeight: 700, color: c.same ? P.good : P.ink, fontFamily: P.fontMono }}>{c.eta}<span style={{ fontSize: 10, color: P.inkMute, fontWeight: 500 }}> min</span></div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, justifyContent: 'flex-end', marginTop: 3 }}><RiskBar score={c.score} width={44} height={4} /><span style={{ fontSize: 10, color: P.inkMute, fontFamily: P.fontMono }}>{c.score}</span></div>
          </div>
        </button>; })}
    </div>
...
window.LAlertRow = function LAlertRow({ a, onAct, compact }) {
  ...
  return <div style={{ display: 'flex', alignItems: 'flex-start', gap: 11, padding: '12px 14px', background: P.surface2, border: `1px solid ${P.hairline2}`, borderLeft: `3px solid ${c}`, borderRadius: P.r12 }}>
    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}><span style={{ fontSize: 13.5, fontWeight: 700, color: P.ink, lineHeight: 1.25 }}>{a.title}</span><span style={{ fontSize: 10, color: P.inkMute, fontFamily: P.fontMono, marginLeft: 'auto', flex: '0 0 auto' }}>{a.at}</span></div>
...
window.LRegionCard = function LRegionCard({ stat, drivers, onReassign, onFlash, onAct, onOpen, variant = 'orders' }) {
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', background: P.surface, border: `1px solid ${stat.health === 'bad' ? hc : P.hairline2}`, borderRadius: P.r14, overflow: 'hidden' }}>
    ...
    {(() => { const bm = stat.ds.filter((d) => d.status === 'break' || d.status === 'meal'); if (!bm.length) return null; return <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '7px 14px', background: P.surface2, borderBottom: `1px solid ${P.hairline}` }}><Icon name="clock" size={13} color={P.info} /><span style={{ fontSize: 11.5, color: P.inkDim, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{bm.map((d) => `${d.name.split(' ')[0]} · ${d.status === 'meal' ? (d.mealType || 'meal') : 'on break'}`).join(' · ')}</span></div>; })()}
    <div style={{ display: 'flex', gap: 14, padding: '11px 14px' }}>
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: P.inkMute, marginBottom: 4 }}>Load</div>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}><span style={{ fontSize: 21, fontWeight: 700, color: hc, fontFamily: P.fontMono }}>{stat.demand}</span><span style={{ fontSize: 11.5, color: P.inkDim }}>orders / {stat.driversOn} driver{stat.driversOn !== 1 ? 's' : ''}</span></div>
      ...
      <div style={{ flex: '0 0 auto', textAlign: 'right' }} title="The region's single order closest to missing its SLA window. The % is that order's on-time confidence — lower means more urgent.">
        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: P.inkMute, marginBottom: 4 }}>Riskiest order</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'flex-end' }}><RiskDot score={stat.worst} /><span style={{ fontSize: 13.5, fontWeight: 700, color: L.riskColor(P, stat.band), fontFamily: P.fontMono }}>{Math.round(stat.worst * 100)}%</span></div>
        <div style={{ fontSize: 10, color: P.inkMute, marginTop: 2 }}>on-time chance</div>
      </div>
    </div>
    <div style={{ padding: '2px 14px 10px', display: 'flex', flexDirection: 'column', gap: 6 }}>
      {variant === 'drivers' && (() => { const rd = stat.ds.filter((d) => d.status !== 'oos'); return <>
        {rd.length === 0 && <span style={{ fontSize: 11.5, color: P.inkMute }}>No drivers on shift</span>}
        {rd.slice(0, 4).map((d) => { const bm = d.status === 'break' || d.status === 'meal'; const sc = d.status === 'idle' ? P.warn : bm ? P.info : P.good; const oc = d.load >= 3 ? P.bad : d.load >= 2 ? P.warn : P.good; return <div key={d.id} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11.5 }}>
          ...
          {bm ? <span style={{ fontSize: 10, fontWeight: 700, color: P.info }}>{d.status === 'meal' ? 'Meal' : 'Break'}</span> : <><BarMeter value={d.load} max={4} width={46} height={4} color={oc} /><span style={{ fontFamily: P.fontMono, fontSize: 10, color: P.inkDim }}>{d.load}/4</span></>}
        </div>; })}
      </>; })()}
      {variant === 'sla' && <>
        {assigned.length === 0 && <span style={{ fontSize: 11.5, color: P.inkMute }}>No active stops</span>}
      ...
      {variant === 'orders' && <>
        {assigned.slice(0, 3).map((o) => <div key={o.id} onClick={() => onOpen && onOpen(o.id)} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11.5, cursor: onOpen ? 'pointer' : 'default' }}>
          ...
          {o.late ? <span style={{ color: P.bad, fontWeight: 700, fontFamily: P.fontMono, fontSize: 11.5 }}>+{o.late}m</span> : <span style={{ color: P.inkMute, fontFamily: P.fontMono, fontSize: 11.5 }}>{o.deadline}</span>}
        </div>)}
        {assigned.length === 0 && <span style={{ fontSize: 11.5, color: P.inkMute }}>No active stops</span>}
        {assigned.length > 3 && <span style={{ fontSize: 11.5, color: P.inkMute }}>+{assigned.length - 3} more</span>}
      </>}
    </div>
