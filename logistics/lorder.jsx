...
function CatPCard({ p, rec, qty, onQty }) {
  ...
  return <div style={{ display: 'flex', gap: 10, padding: 9, background: P.surface2, border: `1px solid ${qty ? P.accentBorder : P.hairline}`, borderRadius: P.r10 }}>
    <div style={{ position: 'relative', flex: '0 0 auto' }}><Thumb item={p} size={46} radius={9} />{p.was && <span style={{ position: 'absolute', top: -5, left: -5, padding: '1px 5px', borderRadius: 6, background: P.accent, color: P.accentInk, fontSize: 9, fontWeight: 800 }}>-{Math.round((1 - p.price / p.was) * 100)}%</span>}</div>
    <div style={{ flex: 1, minWidth: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <span style={{ fontSize: 8.5, fontWeight: 800, letterSpacing: '.04em', textTransform: 'uppercase', color: catColor(p.cat), background: catColor(p.cat) + '22', borderRadius: 5, padding: '1px 6px', flex: '0 0 auto' }}>{p.cat}</span>
        {p.thc != null && <span style={{ fontSize: 9.5, fontWeight: 700, color: P.ink2, fontFamily: P.fontMono }}>{p.thc}% THC</span>}
      </div>
      <div style={{ fontSize: 12, fontWeight: 600, color: P.ink, lineHeight: 1.2, marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.name}</div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 2, fontSize: 9.5, color: P.inkMute, fontFamily: P.fontMono, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}><span>{p.brand}</span><span style={{ opacity: .5 }}>·</span><Icon name="box" size={10} color={P.inkMute} /><span>{L.boxOf(p)}</span></div>
      {rec ? <div style={{ fontSize: 9.5, color: P.warn, fontWeight: 600, marginTop: 3 }}>{p._reason}</div> : <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 3 }}><span style={{ fontSize: 12, fontWeight: 700, color: P.ink, fontFamily: P.fontMono }}>{money(p.price)}</span>{p.was && <span style={{ fontSize: 10, color: P.inkMute, textDecoration: 'line-through', fontFamily: P.fontMono }}>{money(p.was)}</span>}</div>}
    </div>
  ...
  return <div ref={ref} style={{ position: 'relative', width: '100%', height: '100%', borderRadius: P.r16, overflow: 'hidden', border: `1px solid ${P.hairline2}`, background: '#101014' }}>
    ...
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
      {L.REGIONS.map((r) => {const c = L.RMAP[r.code];if (!c) return null;return <div key={'lbl' + r.code} style={{ position: 'absolute', left: c[0] * 100 + '%', top: c[1] * 100 + '%', transform: 'translate(-50%,-50%)', display: 'inline-flex', alignItems: 'center', gap: 5, padding: '2px 7px', borderRadius: 99, background: 'rgba(12,12,10,.62)', border: `1px solid ${L.regionColor(r.code)}`, whiteSpace: 'nowrap', backdropFilter: 'blur(2px)' }}>
        <span style={{ width: 6, height: 6, borderRadius: 2, background: L.regionColor(r.code) }} /><span style={{ fontFamily: P.fontMono, fontSize: 9.5, fontWeight: 800, color: '#fff' }}>{r.code}</span><span style={{ fontSize: 9.5, color: 'rgba(255,255,255,.72)' }}>{r.city}</span>
      </div>;})}
      {showDrivers && drivers.filter((d) => ['duty', 'idle', 'break', 'meal'].includes(d.status)).map((d) => {
        ...
        return <div key={'d' + d.id} style={{ position: 'absolute', left: d.mx * 100 + '%', top: d.my * 100 + '%', transform: 'translate(-50%,-50%)', pointerEvents: 'auto' }} title={brk ? `${d.name} · ${d.status === 'meal' ? d.mealType || 'meal' : 'on break'}` : `${d.name} · ${d.load} stops`}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '3px 6px 3px 4px', background: brk ? P.info : d.status === 'idle' ? P.warn : c, borderRadius: 99, boxShadow: '0 2px 6px rgba(0,0,0,.5)', border: '2px solid rgba(255,255,255,.7)', opacity: brk ? .92 : 1 }}>
            {brk ? <Icon name="clock" size={12} color="#fff" stroke={2.2} /> : <><Icon name="truck" size={11} color="#fff" stroke={2} /><span style={{ fontSize: 9.5, fontWeight: 800, color: '#fff', fontFamily: P.fontMono }}>{d.load}</span></>}
          </div>
        </div>;
      })}
      {active.map((o) => {
        ...
        return <div key={o.id} onClick={(e) => {e.stopPropagation();onSelect(o.id);}} style={{ position: 'absolute', left: o.mx * 100 + '%', top: o.my * 100 + '%', transform: `translate(-50%,-100%) scale(${on ? 1.2 : 1})`, pointerEvents: 'auto', cursor: 'pointer', zIndex: on ? 30 : un ? 20 : 10, transition: 'transform .12s' }}>
          <div style={{ position: 'relative', width: 30, height: 38, filter: 'drop-shadow(0 3px 5px rgba(0,0,0,.55))' }}>
            ...
            <span style={{ position: 'absolute', top: 5, left: 0, right: 0, textAlign: 'center', fontSize: 12, fontWeight: 800, color: un ? '#fff' : '#0b0b08', fontFamily: P.fontMono }}>{un ? '!' : num}</span>
          </div>
        </div>;
      })}
    ...
    <div style={{ position: 'absolute', right: 12, bottom: 12, display: 'flex', flexDirection: 'column', gap: 6, padding: '9px 12px', background: 'rgba(12,12,10,.84)', border: '1px solid rgba(255,255,255,.12)', borderRadius: 12, backdropFilter: 'blur(6px)' }}>
      {[[P.bad, 'Unassigned / at risk'], [P.warn, 'Tight SLA'], [P.good, 'On track']].map(([col, lb]) => <span key={lb} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 10.5, color: 'rgba(255,255,255,.8)' }}><span style={{ width: 9, height: 9, borderRadius: 99, background: col }} />{lb}</span>)}
      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, fontSize: 10.5, color: 'rgba(255,255,255,.8)' }}><span style={{ width: 14, borderTop: '2px dashed rgba(255,255,255,.7)' }} />driver route</span>
    </div>
  </div>;
};
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
    {cands.map((c) => {const dis = !c.stock;
      ...
      onMouseEnter={(e) => !dis && (e.currentTarget.style.borderColor = P.accentBorder)} onMouseLeave={(e) => e.currentTarget.style.borderColor = P.hairline2}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 12.5, fontWeight: 600, color: P.ink, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.d.name}{c.idle && <span style={{ fontSize: 9, fontWeight: 700, color: P.warn, marginLeft: 6 }}>IDLE</span>}</div>
        ...
        {c.stock && <div style={{ textAlign: 'right' }}><div style={{ fontSize: 13.5, fontWeight: 700, color: c.same ? P.good : P.ink, fontFamily: P.fontMono }}>{c.eta}<span style={{ fontSize: 9, color: P.inkMute }}>m</span></div><div style={{ marginTop: 2 }}><RiskBar score={c.score} width={38} height={4} /></div></div>}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      {alts.map((p) => {const dThc = (p.thc || 0) - curThc;const dPrice = +(p.price - curPrice).toFixed(2);
        return <button key={p.sku} onClick={() => onSwap(p.sku)} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '7px 8px', background: P.surface2, border: `1px solid ${P.hairline}`, borderRadius: P.r8, cursor: 'pointer', textAlign: 'left', fontFamily: P.fontSans }} onMouseEnter={(e) => e.currentTarget.style.borderColor = P.accentBorder} onMouseLeave={(e) => e.currentTarget.style.borderColor = P.hairline}>
          ...
          <span style={{ fontSize: 10.5, fontWeight: 700, color: P.accent, flex: '0 0 auto' }}>Use</span>
        </button>;})}
      {alts.length === 0 && <div style={{ fontSize: 11, color: P.inkMute, padding: 10, textAlign: 'center' }}>{mode === 'potent' ? 'Nothing more potent in stock' : mode === 'cheaper' ? 'Nothing cheaper in stock' : 'No alternatives in this category'}</div>}
    </div>
  </div>;
}
...
function LCatalog({ order, onItems, wide }) {
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
    ...
    {recs.length > 0 && !forCust && !saleOnly && cat === 'All' && !q && <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 7 }}><Icon name="sparkle" size={13} color="#E5A24E" /><span style={{ fontSize: 11, fontWeight: 700, color: P.ink2 }}>Recommended for {cust}</span><span style={{ fontSize: 10, color: P.inkMute }}>· from order history</span></div>
    ...
    {list.length === 0 && <div style={{ textAlign: 'center', color: P.inkMute, fontSize: 12, padding: 20 }}>No products found</div>}
  </div>;
}
...
function StatTile({ label, value, color, icon, note }) {
  ...
  return <div style={{ flex: 1, minWidth: 0, position: 'relative', padding: '11px 12px 10px 14px', background: P.surface2, border: `1px solid ${P.hairline2}`, borderRadius: P.r12, overflow: 'hidden' }}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 7 }}>
      ...
      <span style={{ fontSize: 8.5, fontWeight: 800, letterSpacing: '.07em', textTransform: 'uppercase', color: P.inkMute, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{label}</span>
    </div>
    ...
    {note && <div style={{ fontSize: 9, color: P.inkMute, marginTop: 3 }}>{note}</div>}
  </div>;
}
  ...
  return <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: P.scrim, zIndex: 400, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
    <div onClick={(e) => e.stopPropagation()} style={{ width: 'min(468px,96%)', maxHeight: '90%', display: 'flex', flexDirection: 'column', background: P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r16, boxShadow: P.shadowLg, overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '14px 18px', borderBottom: `1px solid ${P.hairline}` }}>
        ...
        <div style={{ flex: 1 }}><div style={{ fontSize: 15, fontWeight: 700, color: P.ink }}>{headTitle} #{o.id}</div><div style={{ fontSize: 11, color: P.inkDim }}>{o.recipient} · {o.addr}</div></div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 18, display: 'flex', flexDirection: 'column', gap: 12 }}>
        {!isCancel && !winPicker && <>
          ...
          {opts.map((r) => <button key={r.k} onClick={() => pickResched(r)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 14px', background: P.surface2, border: `1px solid ${P.hairline2}`, borderRadius: P.r12, cursor: 'pointer', textAlign: 'left', fontFamily: P.fontSans }} onMouseEnter={(e) => e.currentTarget.style.borderColor = r.col} onMouseLeave={(e) => e.currentTarget.style.borderColor = P.hairline2}>
            ...
            <div style={{ flex: 1 }}><div style={{ fontSize: 13.5, fontWeight: 700, color: P.ink }}>{r.label}</div><div style={{ fontSize: 11, color: P.inkDim, marginTop: 1 }}>{r.sub}</div></div>
            <Icon name="chevron-right" size={16} color={P.inkFaint} />
          </button>)}
        </>}
        {!isCancel && winPicker && <>
          {schedUnassigned && <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '10px 12px', background: P.infoSoft, borderRadius: P.r10 }}><Icon name="info" size={14} color={P.info} style={{ marginTop: 1, flex: '0 0 auto' }} /><span style={{ fontSize: 11, color: P.ink2, lineHeight: 1.45 }}>No driver yet — this order auto-assigns ~30 min before the window opens. Set the new window below.</span></div>}
          <div><Lbl>Date</Lbl><div style={{ display: 'flex', gap: 6 }}>{RESCHED_DATES.map((d) => {const on = date === d.k;return <button key={d.k} onClick={() => setDate(d.k)} style={{ flex: 1, padding: '9px 6px', background: on ? P.accentSoft : P.surface2, border: `1px solid ${on ? P.accentBorder : P.hairline2}`, borderRadius: P.r10, cursor: 'pointer', fontFamily: P.fontSans }}><div style={{ fontSize: 12.5, fontWeight: 700, color: on ? (P.mode === 'dark' ? P.accent : '#7A5A00') : P.ink }}>{d.d}</div><div style={{ fontSize: 9.5, color: P.inkMute, fontFamily: P.fontMono, marginTop: 1 }}>{d.full.split(', ')[1]}</div></button>;})}</div></div>
        ...
        {isCancel && <>
          ...
          <div style={{ padding: '12px 13px', background: noteKind === 'hostile' ? P.badSoft : P.warnSoft, border: `1px solid ${noteKind === 'hostile' ? P.bad : P.warn}`, borderRadius: P.r12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 8 }}><Icon name="flag" size={14} color={noteKind === 'hostile' ? P.bad : P.warn} /><span style={{ fontSize: 11, fontWeight: 800, letterSpacing: '.04em', textTransform: 'uppercase', color: noteKind === 'hostile' ? P.bad : P.warn }}>Hot note</span><span style={{ fontSize: 10.5, color: P.inkDim }}>pins to the top of {o.recipient}’s profile</span></div>
            <div style={{ display: 'flex', gap: 5, marginBottom: 8, flexWrap: 'wrap' }}>{chips.map(([tx]) => <button key={tx} onClick={() => setNote(tx)} style={{ padding: '3px 8px', borderRadius: 99, border: `1px solid ${P.hairline2}`, background: P.surface, color: P.ink2, fontSize: 10.5, fontWeight: 600, cursor: 'pointer', fontFamily: P.fontSans }}>{tx}</button>)}</div>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 9, padding: '13px 18px', borderTop: `1px solid ${P.hairline}` }}>
        ...
        {isCancel && <PBtn variant="accent" size="md" icon="user-off" onClick={confirmCancel} style={{ background: P.bad, color: '#fff', opacity: reason ? 1 : .5 }}>Cancel order</PBtn>}
...
window.LCustomerProfile = function LCustomerProfile({ name, onClose, onFlash }) {
  ...
  const Stat = ({ k, v }) => <div style={{ padding: '9px 11px', background: P.surface2, border: `1px solid ${P.hairline2}`, borderRadius: P.r10 }}><div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.05em', textTransform: 'uppercase', color: P.inkMute }}>{k}</div><div style={{ fontSize: 13.5, fontWeight: 700, color: P.ink, marginTop: 2 }}>{v}</div></div>;
  return <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: P.scrim, zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
    <div onClick={(e) => e.stopPropagation()} style={{ width: 'min(540px,96%)', maxHeight: '90%', display: 'flex', flexDirection: 'column', background: P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r20, boxShadow: P.shadowLg, overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 18px', borderBottom: `1px solid ${P.hairline}` }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: 17, fontWeight: 800, color: P.ink, letterSpacing: '-.01em' }}>{name}</span><span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: vip ? '4px 11px' : '3px 9px', borderRadius: 99, background: vip ? tier.c : tier.c + '22', color: vip ? '#1A1400' : tier.c, fontSize: vip ? 11.5 : 10.5, fontWeight: 800, boxShadow: vip ? `0 2px 10px ${tier.c}66` : 'none', border: vip ? 'none' : `1px solid ${tier.c}55` }}><Icon name={tier.ic} size={vip ? 13 : 11} stroke={2.2} />{cust.tier}</span>{notes.length > 0 && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, padding: '2px 7px', borderRadius: 99, background: P.bad, color: '#fff', fontSize: 9.5, fontWeight: 800 }}><Icon name="flag" size={9} stroke={2.4} />{notes.length} flag{notes.length > 1 ? 's' : ''}</span>}</div><div style={{ fontSize: 12, color: P.inkDim, fontFamily: P.fontMono, marginTop: 2 }}>{cust.phone} · member since {cust.joined}</div></div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
        {notes.length > 0 && <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}><div style={{ fontSize: 10, fontWeight: 800, letterSpacing: '.06em', textTransform: 'uppercase', color: P.bad }}>Behaviour flags</div>{notes.map((n, i) => <div key={i} style={{ display: 'flex', gap: 9, padding: '10px 12px', background: n.kind === 'hostile' ? P.badSoft : P.warnSoft, border: `1px solid ${n.kind === 'hostile' ? P.bad : P.warn}`, borderRadius: P.r10 }}><Icon name="flag" size={15} color={n.kind === 'hostile' ? P.bad : P.warn} style={{ flex: '0 0 auto', marginTop: 1 }} /><div style={{ minWidth: 0 }}><div style={{ fontSize: 9.5, fontWeight: 800, letterSpacing: '.05em', textTransform: 'uppercase', color: n.kind === 'hostile' ? P.bad : P.warn }}>{n.kind === 'hostile' ? 'Hostile / unsafe' : n.kind === 'noshow' ? 'No-show' : 'Hot note'} · {n.at}{n.reason ? ' · ' + n.reason : ''}</div><div style={{ fontSize: 12, color: P.ink2, marginTop: 2, lineHeight: 1.45 }}>{n.text}</div></div></div>)}</div>}
        ...
        {cust.note && cust.note !== '—' && <div style={{ padding: '11px 13px', background: P.surface2, border: `1px solid ${P.hairline2}`, borderRadius: P.r10 }}><div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: '.05em', textTransform: 'uppercase', color: P.inkMute, marginBottom: 4 }}>Preferences</div><div style={{ fontSize: 12.5, color: P.ink2, fontStyle: 'italic', lineHeight: 1.45 }}>“{cust.note}”</div></div>}
        <div><div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.05em', textTransform: 'uppercase', color: P.inkMute, marginBottom: 8 }}>Orders this session · {history.length}</div><div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>{history.map((o) => <div key={o.id} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '8px 10px', background: P.surface2, border: `1px solid ${P.hairline}`, borderRadius: P.r10 }}><span style={{ fontFamily: P.fontMono, fontSize: 11.5, fontWeight: 700, color: P.ink }}>#{o.id}</span><RegionTag code={o.region} size="sm" /><span style={{ fontSize: 11.5, color: P.ink2, flex: 1, minWidth: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{o.addr}</span>{o.sched ? <span style={{ fontSize: 10, fontWeight: 700, color: P.info, fontFamily: P.fontMono }}>{o.win}</span> : o.late ? <span style={{ fontSize: 10.5, fontWeight: 700, color: P.bad, fontFamily: P.fontMono }}>+{o.late}m</span> : <span style={{ fontSize: 10.5, fontWeight: 700, color: P.good, fontFamily: P.fontMono }}>on track</span>}</div>)}{history.length === 0 && <div style={{ fontSize: 11.5, color: P.inkMute, padding: 8 }}>No active orders.</div>}</div></div>
      </div>
...
window.LOrderDetail = function LOrderDetail({ order, drivers, onReassign, onItems, onFlash, onClose, wide }) {
  ...
  return <>
    ...
    <div style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
      ...
      <Sec>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <button onClick={() => setProfileOpen((v) => !v)} style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'transparent', border: 'none', cursor: 'pointer', padding: 0, textAlign: 'left', flex: 1, minWidth: 0 }}>
            ...
            <div style={{ minWidth: 0 }}><div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ fontSize: 14, fontWeight: 700, color: P.ink }}>{o.recipient}</span>{notes.length > 0 && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, padding: '1px 6px', borderRadius: 99, background: P.bad, color: '#fff', fontSize: 9, fontWeight: 800 }}><Icon name="flag" size={9} stroke={2.4} />{notes.length}</span>}<Icon name="chevron-down" size={13} color={P.inkMute} style={{ transform: profileOpen ? 'rotate(180deg)' : 'none' }} /></div><div style={{ fontSize: 11, color: P.inkDim, fontFamily: P.fontMono }}>{cust.phone} · Txn {o.txn}</div></div>
          </button>
        ...
        {profileOpen && <div style={{ marginTop: 9, padding: '11px 12px', background: P.surface2, border: `1px solid ${P.hairline2}`, borderRadius: P.r10 }}>
          {notes.length > 0 && <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 11 }}>{notes.map((n, i) => <div key={i} style={{ display: 'flex', gap: 8, padding: '8px 10px', background: n.kind === 'hostile' ? P.badSoft : P.warnSoft, border: `1px solid ${n.kind === 'hostile' ? P.bad : P.warn}`, borderRadius: P.r8 }}><Icon name="flag" size={14} color={n.kind === 'hostile' ? P.bad : P.warn} style={{ flex: '0 0 auto', marginTop: 1 }} /><div style={{ minWidth: 0 }}><div style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.05em', textTransform: 'uppercase', color: n.kind === 'hostile' ? P.bad : P.warn }}>{n.kind === 'hostile' ? 'Hostile' : n.kind === 'noshow' ? 'No-show' : 'Hot note'} · {n.at}</div><div style={{ fontSize: 11.5, color: P.ink2, marginTop: 2, lineHeight: 1.4 }}>{n.text}</div></div></div>)}</div>}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '9px 12px' }}>
            {[['Orders', cust.orders], ['Lifetime value', money(cust.ltv)], ['Last order', cust.last], ['Favorite', cust.fav], ['Member since', cust.joined]].map(([k, v]) => <div key={k}><div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.05em', textTransform: 'uppercase', color: P.inkMute }}>{k}</div><div style={{ fontSize: 12, fontWeight: 600, color: P.ink2, marginTop: 1 }}>{v}</div></div>)}
          </div>
          {cust.note && cust.note !== '—' && <div style={{ marginTop: 9, fontSize: 11, color: P.inkDim, fontStyle: 'italic', lineHeight: 1.4 }}>“{cust.note}”</div>}
      ...
      <Sec bt>
        <Lbl>Destination</Lbl>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}><Icon name="pin" size={15} color={P.inkMute} style={{ marginTop: 1, flex: '0 0 auto' }} /><span style={{ fontSize: 13, color: P.ink2 }}>{o.addr}</span></div>
      </Sec>
      {/* driver + inventory-aware reassign */}
      <Sec bt>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ flex: 1, minWidth: 0 }}><Lbl>Driver</Lbl>{d ? <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Avatar name={d.name} size={30} /><div style={{ minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 700, color: P.ink }}>{d.name}</div><div style={{ fontSize: 11, color: P.inkDim, fontFamily: P.fontMono }}>{d.phone}</div></div>{o.sched && <span style={{ marginLeft: 2, fontSize: 9.5, fontWeight: 700, color: P.info }}>auto-confirmed before window</span>}</div> : o.sched ? <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '9px 11px', background: P.surface2, border: `1px solid ${P.info}`, borderRadius: P.r10 }}><Icon name="calendar" size={15} color={P.info} style={{ flex: '0 0 auto', marginTop: 1 }} /><div><div style={{ fontSize: 12.5, fontWeight: 700, color: P.info }}>Auto-assign scheduled</div><div style={{ fontSize: 10.5, color: P.ink2, lineHeight: 1.45, marginTop: 2 }}>{o.noCandReason || `A driver is assigned automatically ~30 min before the ${future ? 'window opens tomorrow' : 'delivery window'}.`}</div></div></div> : <div><div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 700, color: P.bad }}><Icon name="user-off" size={16} />No driver assigned</div><div style={{ display: 'flex', alignItems: 'flex-start', gap: 7, marginTop: 6, padding: '7px 10px', background: P.badSoft, borderRadius: P.r8 }}><Icon name="info" size={13} color={P.bad} style={{ marginTop: 1, flex: '0 0 auto' }} /><span style={{ fontSize: 10.5, color: P.ink2, lineHeight: 1.45 }}><b style={{ color: P.bad }}>Why:</b> {o.noCandReason || 'Awaiting the next auto-assign run.'}</span></div></div>}</div>
        ...
        {reassignOpen && !future && <div style={{ marginTop: 10 }}><div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 7 }}><Icon name="sparkle" size={12} color={P.accent} /><span style={{ fontSize: 10.5, color: P.inkMute }}>In-stock drivers ranked by ETA · score · load</span></div><InlineReassign order={o} drivers={drivers} onReassign={onReassign} onFlash={onFlash} /></div>}
      </Sec>
      {/* schedule tiles */}
      <Sec bt>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
          {t.line.map((l) => <div key={l.sku}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 9px', background: P.surface2, border: `1px solid ${P.hairline}`, borderRadius: P.r10 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 12.5, fontWeight: 600, color: P.ink, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{l.p ? l.p.name : l.sku}</div><div style={{ fontSize: 11, color: P.inkDim, fontFamily: P.fontMono }}>{l.p ? l.p.brand : ''} · {money(l.ext)}</div></div>
          ...
          {t.line.length === 0 && <div style={{ padding: 14, textAlign: 'center', fontSize: 12, color: P.inkMute, border: `1.5px dashed ${P.hairline3}`, borderRadius: P.r10 }}>No items — add from the catalog</div>}
        </div>
      ...
      <Sec bt>
        ...
        {!promo && <div style={{ marginTop: 10, padding: 11, background: P.accentSoft, border: `1.5px solid ${P.accentBorder}`, borderRadius: P.r12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}><Icon name="tag" size={13} color={P.mode === 'dark' ? P.accent : '#7A5A00'} stroke={2} /><span style={{ fontSize: 11, fontWeight: 800, letterSpacing: '.03em', color: P.mode === 'dark' ? P.accent : '#7A5A00' }}>Promo or referral code</span></div>
        ...
        {promo && <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 8, padding: '6px 10px', background: P.goodSoft, borderRadius: P.r8 }}><Icon name="check-circle" size={13} color={P.good} /><span style={{ fontSize: 11, fontWeight: 700, color: P.good }}>{promo.code}</span><span style={{ fontSize: 11, color: P.ink2 }}>{promo.label}</span><div style={{ flex: 1 }} /><button onClick={() => {setPromo(null);setCode('');}} style={{ background: 'none', border: 'none', color: P.inkMute, cursor: 'pointer', fontSize: 11 }}>Remove</button></div>}
