...
function CatChip({ on, onClick, icon, color, children }) { const P = useP(); return <button onClick={onClick} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '6px 11px', borderRadius: 99, border: `1.5px solid ${on ? color || P.ink : P.hairline2}`, background: on ? color ? color + '22' : P.ink : 'transparent', color: on ? color || P.surface : P.ink2, fontSize: 12.5, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>{icon && <Icon name={icon} size={13} stroke={2} />}{children}</button>; }
function CatPCard({ p, rec, qty, onQty }) {
  const P = useP();
  return <div style={{ display: 'flex', gap: 10, padding: 9, background: P.surface2, border: `1px solid ${qty ? P.ink : P.hairline}`, borderRadius: P.r10 }}>
...
function OrderActionModal({ order, kind, onClose, onFlash, onDone }) {
  ...
  return <div onClick={onClose} style={window.overlayScrim(P, { z: 400, padding: 20 })}>
    <div onClick={(e) => e.stopPropagation()} style={{ ...window.overlayCard, width: 'min(468px,96%)', maxHeight: '90%', display: 'flex', flexDirection: 'column', background: P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r16, boxShadow: P.shadowLg, overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 18, display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {!isCancel && winPicker && <>
          ...
          <div><Lbl>Date</Lbl><div style={{ display: 'flex', gap: 6 }}>{RESCHED_DATES.map((d) => {const on = date === d.k;return <button key={d.k} onClick={() => setDate(d.k)} style={{ flex: 1, padding: '9px 6px', background: on ? P.surface3 : P.surface2, border: `1px solid ${on ? P.ink : P.hairline2}`, borderRadius: P.r10, cursor: 'pointer', fontFamily: P.fontSans }}><div style={{ fontSize: 12.5, fontWeight: 700, color: P.ink }}>{d.d}</div><div style={{ fontSize: 10, color: P.inkMute, fontFamily: P.fontMono, marginTop: 1 }}>{d.full.split(', ')[1]}</div></button>;})}</div></div>
          <div><Lbl>Delivery window</Lbl><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>{RESCHED_WINDOWS.map((w) => {const on = win === w;return <button key={w} onClick={() => setWin(w)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 11px', background: on ? P.surface3 : P.surface2, border: `1px solid ${on ? P.ink : P.hairline2}`, borderRadius: P.r10, cursor: 'pointer', textAlign: 'left', fontFamily: P.fontSans }}><span style={{ width: 13, height: 13, borderRadius: 99, border: `2px solid ${on ? P.ink : P.hairline3}`, background: on ? P.ink : 'transparent', flex: '0 0 auto' }} /><span style={{ fontSize: 11.5, fontWeight: 600, color: P.ink, fontFamily: P.fontMono }}>{w}</span></button>;})}</div></div>
        </>}
        {isCancel && <>
          <div><div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: P.inkMute, marginBottom: 7 }}>Why is this being cancelled?</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>{L.CANCEL_REASONS.map((r) => {const on = reason === r;return <button key={r} onClick={() => setReason(r)} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '9px 10px', background: on ? P.surface3 : P.surface2, border: `1px solid ${on ? P.ink : P.hairline2}`, borderRadius: P.r10, cursor: 'pointer', textAlign: 'left', fontFamily: P.fontSans }}><span style={{ width: 13, height: 13, borderRadius: 99, border: `2px solid ${on ? P.ink : P.hairline3}`, background: on ? P.ink : 'transparent', flex: '0 0 auto' }} /><span style={{ fontSize: 11.5, fontWeight: 600, color: P.ink }}>{r}</span></button>;})}</div>
          </div>
...
window.LCustomerProfile = function LCustomerProfile({ name, onClose, onFlash }) {
  ...
  return <div onClick={onClose} style={window.overlayScrim(P, { z: 500, padding: 20 })}>
    <div onClick={(e) => e.stopPropagation()} style={{ ...window.overlayCard, width: 'min(540px,96%)', maxHeight: '90%', display: 'flex', flexDirection: 'column', background: P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r20, boxShadow: P.shadowLg, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
          {t.line.map((l) => <div key={l.sku}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 9px', background: P.surface2, border: `1px solid ${P.hairline}`, borderRadius: P.r10 }}>
              ...
              <IconBtn icon="swap" size={15} title="Swap product" onClick={() => setSwapFor(swapFor === l.sku ? null : l.sku)} style={{ width: 30, height: 30, color: swapFor === l.sku ? P.ink : P.ink2 }} />
