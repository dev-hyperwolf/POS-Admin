...
test('reassign moves a bound card to a DIFFERENT category, keeping the SAME node — not a same-category node swap', async () => {
  // Owner correction, 2026-09-04: "Reassign" in a Kanban board means "move
  // this card to another column," the same operation dragging it does — not
  // "pick a different Weedmaps type for the same category," which is what
  // this file originally (wrongly) built. This test replaces the old one
  // asserting the wrong behavior.
  const sentTracker = [];
  const m = await mount(function (p, method, body) {
    if (method === 'POST') { sentTracker.push({ p, body }); return [200, { result: { ok: true }, map: payload([WELLNESS, FLOWER]) }]; }
    if (p.startsWith(ROUTE + '/preview')) { return [200, preview({ products_affected: 41, to_path: 'Flower [2]' })]; }
    if (p.startsWith(ROUTE)) { return [200, payload([WELLNESS, FLOWER])]; }
    return [404, {}];
  });
  ...
  assert.equal(sentTracker[0].body.node, 512, 'the SAME Weedmaps node moves — reassign never picks a different type');
  assert.equal(sentTracker[1].p, ROUTE + '/unbind');
  assert.equal(sentTracker[1].body.category, 'Wellness');
  assert.equal(sentTracker[1].body.node, 512);
});

test('a reassign whose REMOVE half is refused leaves BOTH categories claiming it and says so, rather than losing the failure', async () => {
  const bothBound = payload([
    row('Wellness', { binding_source: 'explicit', product_count: 41,
      bindings: [{ node_id: 512, actor: 'ops@hyperwolf.com', at_iso: '2026-07-14T00:00:00Z', broken: false,
        path: [{ id: 512, name: 'Tinctures', parent_id: null }] }] }),
    row('Flower', { binding_source: 'explicit', product_count: 512,
      bindings: [{ node_id: 512, actor: 'test@hyperwolf.com', at_iso: '2026-09-04T00:00:00Z', broken: false,
        path: [{ id: 512, name: 'Tinctures', parent_id: null }] }] })
  ]);
  const m = await mount(function (p, method, body) {
  ...
  const card = q(m.doc, '[data-hw-wm-card="512"]');
  await m.click(q(card, 'button[title="Reassign"]'));
  await m.click(q(q(m.doc, '[data-hw-modal="reassign-target"]'), '[data-hw-reassign-target="Flower"]'));
  await m.click(q(m.doc, '[data-hw-review]'));
  await m.click(q(m.doc, '[data-hw-confirm]'));
  assert.match(m.text(), /now has it too, but removing it from .* was refused/i,
    'a partial move failure must be surfaced in words, not swallowed');
  assert.ok(qa(m.doc, '[data-hw-wm-card="512"]').length >= 2,
    'both categories\' cards for this node must still show — the map reflects reality: both claim it');
});
