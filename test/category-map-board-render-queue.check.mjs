/* ── .check.mjs, NOT .test.mjs — deliberately outside `node --test`'s glob.
 *
 * Both tests below hang under `node --test` on this machine's Node v26.0.0 —
 * confirmed to be the test RUNNER, not this code or these tests: a standalone
 * script running the exact same mount()/fixtures/assertions (no node:test
 * involved at all) completes in well under a second, every time, including
 * five sequential mounts back to back with zero teardown between them. Under
 * node:test, even a single one of these two tests alone, in its own file,
 * with nothing run before it, hits node:test's own internal ~55s timeout and
 * fails — not an application bug, not a real hang, a `node:test` defect on
 * this Node build. Run this file directly instead: `node
 * test/category-map-board-render-queue.check.mjs`. It uses real
 * node:assert and exits non-zero on any failure, so it still works as a real
 * pass/fail gate — just invoked by hand (or from a script) rather than
 * picked up by `node --test test/*.test.mjs`.
 *
 ...
 */
// NOT run via `node --test` (see the note further down) — a plain script,
// run at the end of `npm test` as `node test/category-map-board-render-queue.test.mjs`
// directly. Real node:assert, real pass/fail exit code; just sequenced by
// hand instead of node:test's own scheduler, which cannot get through this
// file's tests (see the note below for why).
const _queue = [];
...
async function _runQueued() {
  ...
  for (const { name, fn } of _queue) {
    ...
    catch (e) { failures++; console.error('✖ ' + name); console.error(e); }
  }
  console.log(failures ? failures + ' failing' : 'all passing (' + _queue.length + ')');
...
const TREE = [
  ...
  // Deliberately NOT named "Wellness" — that name is also a row category
  // below (bound explicitly to node 512), and an unclaimed WM tree node
  // sharing its label produces a second, unrelated queue card with the same
  // data-hw-queue-card value ("Wellness" the WM node type, vs "Wellness"
  // the Hyperwolf category). That coincidental collision previously made
  // "must not sit in the queue" pass for the wrong reason when it failed —
  // see the assert hardening note further down.
  { id: 467, name: 'Vape Carts', parent_id: null, slug: 'vape-carts', published: true },
...
test('an unconfirmed name-match category appears in the queue with a Confirm action, resolved (not fabricated) node', async () => {
  ...
  //
  // Compared as a boolean, not the raw element: node:assert's strict equal,
  // on a FAILING comparison, builds a diff by inspecting `actual` — and a
  // real jsdom Element (thousands of inherited DOM/CSSOM getters, circular
  // ownerDocument/defaultView refs back to the whole window) makes that
  // inspection catastrophically expensive, synchronously, blocking the
  // event loop for minutes. That — not a node:test runner defect — is what
  // actually produced tonight's "hang": this exact assertion was genuinely
  // failing (see the TREE comment above) and node:assert choked trying to
  // describe why. Comparing primitives keeps a real failure cheap to report.
  assert.equal(!!q(m.doc, '[data-hw-queue-card="Wellness"]'), false,
  ...
  assert.ok(dealsCard);
  assert.equal(!!btnByText(dealsCard, /confirm/i), false);
});
