...
function RewardsCard({ P, customer }) {
  ...
  return (
    <Card padding={9} style={{ marginBottom: 10, background: P.surface }} data-tour="rewards-card">
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 7 }}>
        ...
        <div style={{ flex: 1 }} />
        <span style={{ fontSize: 13, fontWeight: 700, color: P.ink, fontFamily: P.fontMono }}>{pts.toLocaleString()} <span style={{ fontSize: 9, color: P.inkMute, fontWeight: 600 }}>PTS</span></span>
      </div>
        ...
        {rewards.map((r) => {
          ...
          return (
            <button key={r.id} disabled={!can} onClick={() => doRedeem(r.id)} title={r.bday ? 'Birthday reward — no points required' : `${r.cost} pts`}
              ...
              <span style={{ fontSize: 9.5, fontWeight: 700, color: P.ink, textAlign: 'center', lineHeight: 1.15, whiteSpace: 'nowrap' }}>{r.short || r.label}</span>
              <span style={{ fontSize: 8.5, fontFamily: P.fontMono, color: can ? goldInk : P.inkFaint, whiteSpace: 'nowrap' }}>{r.bday ? 'birthday' : r.cost}</span>
            </button>);
        })}
      </div>

      {redeemed &&
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginTop: 7, fontSize: 11, color: goldInk, fontWeight: 600 }}>
...
function DiscountCard({ P, discMode, setDiscMode, subtotal }) {
  ...
  return (
    <Card padding={9} style={{ marginBottom: 10, background: P.surface }} data-tour="disc-card">
      ...
      <button onClick={() => setPromoOpen(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, marginTop: 6, background: 'none', border: 'none', color: P.inkDim, fontSize: 11.5, fontWeight: 600, cursor: 'pointer', fontFamily: P.fontSans, padding: 0 }}><Icon name="plus" size={12} stroke={2.2} />Add promo code</button>}
...
function AovBooster({ P, total, goal, gap, goalPct, recs, onAdd }) {
  ...
  return (
    <div style={{ marginBottom: 10, border: `1px solid ${met ? P.goodSoft : P.accentBorder}`, borderRadius: P.r12, overflow: 'hidden', background: P.surface }}>
      <div style={{ padding: '7px 11px 8px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 7, marginBottom: 5 }}>
          ...
          <span style={{ fontSize: 10.5, color: P.inkDim, fontFamily: P.fontMono, flex: '0 0 auto', whiteSpace: 'nowrap' }}>{fmt0(total)} / {fmt0(goal)}</span>
        </div>
...
function TaxRow({ P, sub, open, onToggle }) {
  ...
  return (
    <>
      <button onClick={onToggle} aria-expanded={open} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, width: '100%', padding: 0, background: 'none', border: 'none', cursor: 'pointer', fontFamily: P.fontSans, fontSize: 11.5, color: P.inkDim, textAlign: 'left' }}>
      ...
      {open && <div style={{ display: 'flex', flexDirection: 'column', gap: 2, padding: '2px 0 1px 10px', borderLeft: `1px solid ${P.hairline2}`, marginLeft: 1 }}>
        {tb.lines.map((t) => <div key={t.k} style={{ display: 'flex', justifyContent: 'space-between', gap: 10, fontSize: 10.5, color: P.inkMute }}><span style={{ whiteSpace: 'nowrap' }}>{t.k}</span><span style={{ fontFamily: P.fontMono, whiteSpace: 'nowrap' }}>{money(t.v)}</span></div>)}
      </div>}
    </>);
}
