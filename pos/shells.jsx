...
;(function () {
  ...
  const Ey = ({ children, style }) => {
    const P = useP();
    return <span style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: '.11em', textTransform: 'uppercase', color: P.inkMute, fontFamily: P.fontMono, ...style }}>{children}</span>;
  };
      ...
      <Card key={s.id} hover padding={0} onClick={() => onOpen(s.id)} style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ display: 'flex', gap: 13, padding: '16px 16px 14px' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 800, color: P.ink, letterSpacing: '-.02em', lineHeight: 1.05, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.brand}</div>
            <div style={{ fontSize: 13, fontWeight: 600, color: P.ink2, marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{s.sub} · {s.weight}</div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 8, padding: '0 16px 14px' }}>
          ...
          <div key={k} style={{ flex: 1, background: P.surface2, border: `1px solid ${P.hairline}`, borderRadius: 9, padding: '8px 10px' }}>
            <Ey style={{ fontSize: 9 }}>{k}</Ey>
            <div style={{ fontSize: 16, fontWeight: 700, color: P.ink, fontFamily: P.fontMono, marginTop: 2 }}>{v}</div>
            {k === 'Sells at' && s.sale ? <div style={{ fontSize: 9.5, color: P.inkFaint, fontFamily: P.fontMono, textDecoration: 'line-through' }}>{money(s.price)}</div> : null}
          </div>)}
        </div>
        <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, padding: '11px 16px', borderTop: `1px solid ${P.hairline}`, background: P.surface2 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, minWidth: 0 }}>
            ...
            {s.variations.length > 3 && <span style={{ fontSize: 11, color: P.inkMute, fontFamily: P.fontMono }}>+{s.variations.length - 3}</span>}
          </div>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12, fontWeight: 600, color: P.mode === 'dark' ? P.accent : '#7A5A00', flex: '0 0 auto' }}>Open shell<Icon name="chevron-right" size={14} stroke={2.2} /></span>
        </div>
      </Card>;
    ...
    return <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 11, marginBottom: 18 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1 }} />
          <span style={{ fontSize: 12, color: P.inkDim, fontFamily: P.fontMono }}>{filtered.length} of {shells.length} shells</span>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 30 }}>
        ...
        <section key={g.cat}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingBottom: 10, marginBottom: 16, borderBottom: `1px solid ${P.hairline2}` }}>
            <CatDot cat={g.cat} />
            <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700, letterSpacing: '-.01em', color: P.ink }}>{g.cat}</h2>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div key={sg.sub}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 10 }}>
                ...
                <span style={{ fontSize: 10.5, color: P.inkFaint, fontFamily: P.fontMono }}>{sg.list.length}</span>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 16 }}>{sg.list.map(ShellCard)}</div>
            </div>)}
          </div>
        </section>)}
        {groups.length === 0 && <div style={{ padding: 40, textAlign: 'center', fontSize: 13, color: P.inkMute }}>No shells match those filters.</div>}
  ...
  function Detail({ id, onBack, onEdit, onAddVariation }) {
    ...
    const SharedCard = <Card padding={18}>
      ...
      {shared.map((f, i) => <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '10px 12px', marginBottom: 8, background: P.surface2, border: `1px solid ${P.hairline}`, borderRadius: 9 }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', fontSize: 11, fontWeight: 600, color: P.inkDim }}>{f.label}{f.flag && <FTag>{f.flag}</FTag>}</span>
        <span style={{ textAlign: 'right', flex: '0 0 auto' }}>
          <span style={{ display: 'block', fontSize: 13, fontWeight: 700, color: P.ink, fontFamily: P.fontMono }}>{f.value}</span>
          {f.sub && <span style={{ display: 'block', fontSize: 10.5, color: P.inkMute, fontFamily: P.fontMono, marginTop: 2 }}>{f.sub}</span>}
        </span>
      </div>)}
      {(s.traits || []).length > 0 && <div style={{ marginTop: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}><Ey style={{ fontSize: 9 }}>Product traits · carry across the line</Ey>
      ...
      <div style={{ marginTop: 16, padding: 12, background: P.surface2, border: `1px dashed ${P.hairline3}`, borderRadius: P.r10 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 600, color: P.inkDim }}><Icon name="lightning" size={13} />Change once, applies everywhere</div>
    ...
    return <div>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 18, marginBottom: 20 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}><CatDot cat={s.cat} /><span style={{ fontSize: 12, color: P.inkDim, fontFamily: P.fontMono }}>{S.familyPath(s)}</span></div>
          <h1 style={{ margin: 0, fontSize: 28, fontWeight: 700, letterSpacing: '-.02em', color: P.ink, lineHeight: 1.1 }}>{s.name}</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: 20, alignItems: 'start' }}>
        ...
        <div style={{ minWidth: 0 }}>
          ...
          <Card padding={0} style={{ overflow: 'hidden' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1.6fr .8fr .6fr .8fr 44px', gap: 0, padding: '11px 16px', background: P.surface2, borderBottom: `1px solid ${P.hairline2}`, fontSize: 10.5, fontWeight: 600, letterSpacing: '.08em', textTransform: 'uppercase', color: P.inkDim }}>
            ...
            {s.variations.map((v, i) => <div key={v.sku} style={{ display: 'grid', gridTemplateColumns: '1.6fr .8fr .6fr .8fr 44px', gap: 0, alignItems: 'center', padding: '12px 16px', borderTop: i ? `1px solid ${P.hairline}` : 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 11, minWidth: 0 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 10.5, color: P.inkMute, fontFamily: P.fontMono, marginTop: 1 }}>{v.sku}</div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2 }}>
                ...
                {v.override ? <span style={{ fontSize: 9.5, fontWeight: 800, letterSpacing: '.05em', color: P.warn, fontFamily: P.fontMono }}>OVERRIDE</span> : <span style={{ fontSize: 9.5, color: P.inkFaint, fontFamily: P.fontMono }}>inherited</span>}
              </div>
        ...
        <Card padding={0} style={{ marginBottom: 18 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: 10, padding: 18 }}>
            {shared.map((f, i) => <div key={i} style={{ background: P.surface2, border: `1px solid ${P.hairline}`, borderRadius: P.r10, padding: '11px 13px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Ey style={{ fontSize: 9 }}>{f.label}</Ey>{f.flag && <FTag>{f.flag}</FTag>}</div>
              <div style={{ fontSize: 14, fontWeight: 700, color: P.ink, fontFamily: P.fontMono, marginTop: 4 }}>{f.value}</div>
            ...
            {(s.traits || []).map((t, i) => <div key={'t' + i} style={{ background: P.surface2, border: `1px dashed ${P.hairline2}`, borderRadius: P.r10, padding: '11px 13px' }}>
              <Ey style={{ fontSize: 9 }}>{t.label}</Ey>
              <div style={{ fontSize: 14, fontWeight: 700, color: P.ink, fontFamily: P.fontMono, marginTop: 4 }}>{t.value}</div>
            </div>)}
          </div>
        </Card>
        {VarHead}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: 14 }}>
          {s.variations.map((v) => <Card key={v.sku} hover padding={14}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginTop: 12, paddingTop: 11, borderTop: `1px solid ${P.hairline}` }}>
              <div><Ey style={{ fontSize: 8.5 }}>Stock</Ey><div style={{ fontSize: 13, fontWeight: 600, color: stockColor(v.qty || 0), fontFamily: P.fontMono, marginTop: 2 }}>{v.qty || 0}</div></div>
              <div style={{ textAlign: 'right' }}>
                <span style={{ fontSize: 15, fontWeight: 700, color: P.ink, fontFamily: P.fontMono }}>{money(v.price)}</span>
                <div>{v.override ? <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.05em', color: P.warn, fontFamily: P.fontMono }}>OVERRIDE</span> : <span style={{ fontSize: 9, color: P.inkFaint, fontFamily: P.fontMono }}>inherited</span>}</div>
              </div>
            </div>
          </Card>)}
