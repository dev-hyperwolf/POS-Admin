...
//
// Why delivery needs T2 and not "Persona again": the ID is already verified by
...
//
// Persona is therefore a SUBSTITUTE for T1, not an extra step on top of it. It
// exists for customers who have never walked in. A customer who has been
// scanned in store never sees Persona. Ever.
//
...
function assurance(v) {
  if (!v) return { tier: 0, ...TIERS[0], canStore: false, canDelivery: false, blocker: 'No ID has been seen yet.', next: 'Scan their ID at the counter, or send a Persona link.' };
  const doc = v.doc,ph = v.phone || {},pa = v.persona;
  ...
  if (!docOk && !remoteOk) return { tier: 0, ...TIERS[0], canStore: false, canDelivery: false, blocker: 'No ID has been seen yet.', next: 'Scan their ID at the counter, or send a Persona link.' };
  ...
  return { tier: 2, ...TIERS[2], canStore: true, canDelivery: true, via: remoteOk && !docOk ? 'persona' : doc && doc.where === 'door' ? 'door' : 'in-store' };
}
...
window.IdentityLadder = function IdentityLadder({ v, compact }) {
  ...
  const doc = v && v.doc || {};
  const ph = v && v.phone || {};
  const pa = v && v.persona;
  const rungs = [
  { n: 1, t: 'ID inspected', done: a.tier >= 1,
    d: a.tier >= 1 ?
    pa && pa.status === 'passed' && !doc.scannedAt ? 'Persona remote check passed ' + pa.at :
    ...
    {rungs.map((r, i) => {
      ...
      return <div key={r.n} style={{ display: 'flex', gap: 10 }}>
        ...
        <div style={{ paddingBottom: i < rungs.length - 1 ? compact ? 9 : 12 : 0, minWidth: 0, flex: 1 }}>
          <div style={{ fontSize: 12.5, fontWeight: 700, color: r.done ? P.ink : P.ink2, display: 'flex', alignItems: 'center', gap: 6 }}>{r.t}{r.note && <span style={{ fontSize: 8.5, fontWeight: 800, letterSpacing: '.05em', textTransform: 'uppercase', color: P.info, background: P.infoSoft, borderRadius: 99, padding: '1px 6px' }}>{r.note}</span>}</div>
          <div style={{ fontSize: 11.5, color: P.inkDim, marginTop: 1, lineHeight: 1.45 }}>{r.d}</div>
          {r.meta && <div style={{ fontSize: 10.5, color: P.inkMute, fontFamily: P.fontMono, marginTop: 2 }}>{r.meta}</div>}
        </div>
      </div>;})}
  </div>;
};
...
window.SmsVerifyPanel = function SmsVerifyPanel({ phone, state, sentAt, attempts, onVerified, onLog, compact }) {
  ...
  [{ at: sentAt || '2 min ago', by: 'System · auto', status: 'delivered', receipt: 'carrier ack 1.4s' }]);
  ...
  const resend = () => {
    ...
    onLog && onLog({ who: 'Manisha Saini', role: 'You', action: 'Resent verification SMS to ' + phone, time: 'just now', icon: 'phone' });
  ...
  if (st === 'verified') return <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '10px 12px', background: P.goodSoft, border: `1px solid ${P.good}44`, borderRadius: P.r10 }}>
    ...
    <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 12, fontWeight: 700, color: P.ink }}>Phone confirmed — delivery unlocked</div>
      <div style={{ fontSize: 10.5, color: P.inkDim }}>{phone} is now bound to this account. They will never be asked again.</div></div>
  </div>;
  ...
  return <div style={{ padding: '11px 13px', background: P.warnSoft, border: `1px solid ${P.warn}66`, borderRadius: P.r10 }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 9 }}>
      ...
      <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: '.04em', textTransform: 'uppercase', color: P.warn }}>Pending verification</span>
      <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.05em', textTransform: 'uppercase', color: P.info, background: P.infoSoft, borderRadius: 99, padding: '1px 7px' }}>Delivery only</span>
      <span style={{ marginLeft: 'auto', fontSize: 10.5, color: P.inkDim, fontFamily: P.fontMono }}>{phone}</span>
    </div>
    ...
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '8px 10px', background: P.surface, borderRadius: P.r8, marginBottom: 9 }}>
      ...
      <div style={{ fontSize: 11, color: P.ink2, lineHeight: 1.5, fontFamily: P.fontMono, minWidth: 0 }}>
    ...
    <div style={{ background: P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r8, overflow: 'hidden', marginBottom: 9 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 10px', background: P.surface2, borderBottom: `1px solid ${P.hairline}` }}>
        <span style={{ fontSize: 9.5, fontWeight: 800, letterSpacing: '.06em', textTransform: 'uppercase', color: P.inkMute }}>Send log</span>
      ...
      {log.slice(0, 3).map((a, i) => {
        ...
        return <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px', borderTop: i ? `1px solid ${P.hairline}` : 'none' }}>
          ...
          <span style={{ fontSize: 11, fontFamily: P.fontMono, color: P.ink, flex: '0 0 auto' }}>{a.at}</span>
          <span style={{ fontSize: 10.5, color: P.inkDim, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.by}</span>
          <span style={{ marginLeft: 'auto', fontSize: 10, fontWeight: 700, color: c, flex: '0 0 auto' }}>{m.label}</span>
          <span style={{ fontSize: 9.5, color: P.inkMute, fontFamily: P.fontMono, flex: '0 0 auto' }}>{a.receipt}</span>
        </div>;})}
    </div>

    {last.status === 'failed' &&
    <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 9, fontSize: 11, fontWeight: 600, color: P.bad }}>
        <Icon name="shield" size={12} color={P.bad} />Carrier rejected the number — check it is a mobile line before resending.
      </div>}

    {!compact && <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
      <div style={{ flex: '1 1 110px', minWidth: 100 }}><Field mono value={code} onChange={(e) => setCode(e.target.value.replace(/[^0-9]/g, '').slice(0, 6))} placeholder="000000" size="sm" /></div>
      <PBtn variant="primary" size="sm" icon="check" disabled={code.length < 6} onClick={verify}>Confirm</PBtn>
      <PBtn variant="secondary" size="sm" icon="refresh" disabled={secs > 0} onClick={resend}>{secs > 0 ? secs + 's' : 'Resend'}</PBtn>
    </div>}
    {!compact && <div style={{ fontSize: 10.5, color: P.inkMute, marginTop: 7, lineHeight: 1.45 }}>Sent automatically when a <b style={{ color: P.ink2 }}>delivery</b> order appears for an account with an ID on file but no confirmed phone. Walk-ins are never sent one. Resend only if it never landed — the code box is for reading it back over the phone.</div>}
  </div>;
};
      ...
      <div onClick={(e) => e.stopPropagation()} style={{ width: 'min(430px,96vw)', maxHeight: '88vh', display: 'flex', flexDirection: 'column', background: P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r16, boxShadow: P.shadowLg, overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 18px', borderBottom: `1px solid ${P.hairline}` }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 11, color: P.inkDim, fontFamily: P.fontMono }}>{m.id ? 'Customer ' + m.id : 'Not yet a customer record'}</div>
          </div>
        ...
        <div style={{ padding: 18, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 15 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 9 }}>
            {stats.map(([k, v]) => <div key={k} style={{ padding: '10px 12px', background: P.surface2, borderRadius: P.r10 }}>
              <div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: '.05em', textTransform: 'uppercase', color: P.inkMute }}>{k}</div>
...
window.IdScanPanel = function IdScanPanel({ value, onChange, onLog }) {
  ...
  if (st === 'done') {
    ...
    return <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '11px 13px', background: P.goodSoft, border: `1px solid ${P.good}44`, borderRadius: P.r10 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 10.5, color: P.inkDim, fontFamily: P.fontMono }}>{d.type} {d.num} · expires {d.expires} · age 21+ ✓</div>
      </div>
  ...
  return <div style={{ padding: '13px', background: P.surface2, border: `1px dashed ${P.hairline2}`, borderRadius: P.r10, textAlign: 'center' }}>
    ...
    <div style={{ fontSize: 11, color: P.inkDim, marginTop: 3, lineHeight: 1.45 }}>Reads the PDF417 barcode for name, DOB and expiry, and stores a photo of the document.</div>
...
window.VerifyPolicyCard = function VerifyPolicyCard({ tight }) {
  ...
  { i: 'truck', t: 'Never walked in', d: 'Two options, their choice: a Persona link before the first order, or nothing at all — the driver scans their ID at the door on delivery one, which upgrades the account for every order after it.' },
  ...
  { i: 'shield', t: 'Persona is a substitute, not an extra', d: 'It only runs when no human has ever held the document. A scanned-in-store customer never sees it.' }];
  ...
  return <div style={{ background: P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r12, overflow: 'hidden' }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 13px', background: P.surface2, borderBottom: `1px solid ${P.hairline}` }}>
      ...
      <span style={{ fontSize: 11, fontWeight: 800, letterSpacing: '.05em', textTransform: 'uppercase', color: P.ink2 }}>Verify once — never twice</span>
    </div>
    <div style={{ padding: tight ? '10px 13px' : '13px', display: 'flex', flexDirection: 'column', gap: 11 }}>
      {rows.map((r) => <div key={r.t} style={{ display: 'flex', gap: 10 }}>
        ...
        <div><div style={{ fontSize: 12, fontWeight: 700, color: P.ink }}>{r.t}</div>
