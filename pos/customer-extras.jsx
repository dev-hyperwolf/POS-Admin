...
window.HotNotesBanner = function HotNotesBanner({ notes, onAdd, onResolve }) {
  ...
  return <div style={{ border: `1.5px solid ${c}`, background: soft, borderRadius: P.r14, overflow: 'hidden', marginBottom: 16 }} data-tour="hot-notes">
    <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '10px 15px', background: c, color: '#fff' }}>
      ...
      <span style={{ fontSize: 11, fontWeight: 700, fontFamily: P.fontMono, background: 'rgba(255,255,255,.22)', borderRadius: 99, padding: '1px 8px' }}>{notes.length}</span>
      <div style={{ flex: 1 }} />
      {onAdd && <button onClick={onAdd} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '3px 10px', borderRadius: 99, border: '1px solid rgba(255,255,255,.45)', background: 'transparent', color: '#fff', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: P.fontSans }}><Icon name="plus" size={12} stroke={2.6} color="#fff" />Add</button>}
    </div>
        ...
        return <div key={i} style={{ display: 'flex', gap: 11, padding: '12px 15px', borderTop: i ? `1px solid ${c}33` : 'none' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap', marginBottom: 2 }}>
              <span style={{ fontSize: 9.5, fontWeight: 800, letterSpacing: '.05em', textTransform: 'uppercase', color: kc }}>{k.label}</span>
            ...
            <div style={{ fontSize: 10.5, color: P.inkDim, fontFamily: P.fontMono, marginTop: 3 }}>{n.by} · {n.at}</div>
          </div>
          {onResolve && <button onClick={() => onResolve(i)} title="Resolve this note" style={{ flex: '0 0 auto', alignSelf: 'flex-start', padding: '3px 9px', borderRadius: 99, border: `1px solid ${kc}55`, background: '#fff', color: kc, fontSize: 10.5, fontWeight: 700, cursor: 'pointer', fontFamily: P.fontSans }}>Resolve</button>}
        </div>;})}
    </div>
  </div>;
};
  ...
  return <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 200, background: P.scrim, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
    <div onClick={(e) => e.stopPropagation()} style={{ width: 'min(520px,96%)', maxHeight: '90vh', display: 'flex', flexDirection: 'column', background: P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r16, boxShadow: P.shadowLg, overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '15px 20px', borderBottom: `1px solid ${P.hairline}` }}>
        ...
        <div style={{ flex: 1 }}><div style={{ fontSize: 14.5, fontWeight: 700, color: P.ink }}>{hot ? 'Add hot note' : 'Add note'}</div><div style={{ fontSize: 11, color: P.inkDim }}>{member ? member.name : 'Customer'}</div></div>
      ...
      <div style={{ padding: 20, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
        <label onClick={() => setHot((v) => !v)} style={{ display: 'flex', alignItems: 'flex-start', gap: 11, padding: '12px 14px', background: hot ? P.badSoft : P.surface2, border: `1.5px solid ${hot ? P.bad : P.hairline2}`, borderRadius: P.r12, cursor: 'pointer' }}>
          ...
          <div><div style={{ fontSize: 13, fontWeight: 700, color: hot ? P.bad : P.ink }}>Make this a hot note</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            {Object.keys(HOT_KINDS).map((k) => {const m = HOT_KINDS[k];const on = kind === k;const kc = m.tone === 'bad' ? P.bad : m.tone === 'warn' ? P.warn : P.info;
              return <button key={k} onClick={() => setKind(k)} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '9px 11px', textAlign: 'left', background: on ? kc + '18' : P.surface2, border: `1px solid ${on ? kc : P.hairline2}`, borderRadius: P.r10, cursor: 'pointer', fontFamily: P.fontSans }}>
                ...
                <span><span style={{ display: 'block', fontSize: 12, fontWeight: 700, color: on ? kc : P.ink }}>{m.label}</span><span style={{ display: 'block', fontSize: 10.5, color: P.inkDim, lineHeight: 1.4, marginTop: 1 }}>{m.blurb}</span></span>
              </button>;})}
          </div>
        </div>}
        {hot && kind === 'staff' && <div>
          <div style={lbl}>Employee not to be assigned</div>
          <select value={block} onChange={(e) => setBlock(e.target.value)} style={{ width: '100%', padding: '9px 12px', border: `1px solid ${P.hairline2}`, borderRadius: P.r10, background: P.surface, color: P.ink, fontSize: 13, fontWeight: 600, fontFamily: P.fontSans, outline: 'none' }}>
            <option value="">Select an employee…</option>{STAFF.map((s) => <option key={s}>{s}</option>)}
          </select>
          <div style={{ fontSize: 11, color: P.inkDim, marginTop: 5, lineHeight: 1.45 }}>Check-in will warn before this person is put on the sale, and dispatch will skip them when assigning a driver.</div>
        </div>}
        <div>
          <div style={lbl}>{hot ? 'What happened' : 'Note'}</div>
          <textarea autoFocus rows={hot ? 4 : 5} value={text} onChange={(e) => setText(e.target.value)} placeholder={hot ? 'Be specific and factual — dates, what was said or done, who was present.' : 'Add an internal note about this member…'} style={{ width: '100%', boxSizing: 'border-box', resize: 'vertical', padding: '10px 12px', border: `1px solid ${P.hairline2}`, borderRadius: P.r10, background: P.surface, color: P.ink, fontSize: 13, fontFamily: P.fontSans, lineHeight: 1.5, outline: 'none' }} />
        </div>
      </div>
...
window.DeliveryAddressBook = function DeliveryAddressBook({ m, idAddr }) {
  ...
  return <div style={{ border: `1px solid ${P.hairline}`, borderRadius: P.r12, overflow: 'hidden' }} data-tour="addr-book">
    <div style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '10px 13px', background: P.surface2, borderBottom: `1px solid ${P.hairline}` }}>
      ...
      <span style={{ fontSize: 10.5, color: P.inkMute, fontFamily: P.fontMono }}>{list.length}</span>
      <div style={{ flex: 1 }} />
      ...
      return <div key={a.id} style={{ display: 'flex', gap: 11, padding: '12px 13px', borderTop: i ? `1px solid ${P.hairline}` : 'none' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' }}>
            ...
            {a.def && <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '.05em', textTransform: 'uppercase', color: P.accentInk, background: P.accent, borderRadius: 99, padding: '1px 7px' }}>Default</span>}
            <span style={{ fontSize: 9.5, fontWeight: 800, letterSpacing: '.05em', textTransform: 'uppercase', color: z.c }}>{z.label}</span>
          </div>
          <div style={{ fontSize: 12, color: P.ink2, fontFamily: P.fontMono, marginTop: 2 }}>{a.street}, {a.city}, CA {a.zip}</div>
          <div style={{ fontSize: 10.5, color: P.inkMute, marginTop: 3, lineHeight: 1.45 }}>{a.region} · {a.orders} order{a.orders > 1 ? 's' : ''} · last used {a.lastUsed}</div>
          <div style={{ fontSize: 10.5, color: z.c, marginTop: 3, lineHeight: 1.45 }}>{z.note}</div>
        </div>
        {!a.def && <button onClick={() => setDefault(a.id)} style={{ flex: '0 0 auto', alignSelf: 'flex-start', padding: '3px 9px', borderRadius: 99, border: `1px solid ${P.hairline2}`, background: P.surface, color: P.ink2, fontSize: 10.5, fontWeight: 700, cursor: 'pointer', fontFamily: P.fontSans }}>Make default</button>}
      </div>;})}
    {adding && <div style={{ padding: '12px 13px', borderTop: `1px solid ${P.hairline}`, background: P.surface2, display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ fontSize: 10.5, color: P.inkDim, lineHeight: 1.45 }}>The ZIP decides everything: it resolves to a region, the region's on-shift driver decides what can be sold to it, and an unserved ZIP blocks delivery checkout for this address only.</div>
    </div>}
    <div style={{ display: 'flex', gap: 8, padding: '11px 13px', borderTop: `1px solid ${P.hairline}`, background: P.infoSoft }}>
      ...
      <div style={{ fontSize: 11, color: P.ink2, lineHeight: 1.5 }}>
  ...
  return <div style={{ border: `1px solid ${P.hairline}`, borderRadius: P.r12, overflow: 'hidden' }} data-tour="member-verify">
    ...
    <div style={{ padding: 13, display: 'flex', flexDirection: 'column', gap: 11 }}>
      ...
      <button onClick={() => setOpen((v) => !v)} style={{ display: 'flex', alignItems: 'center', gap: 7, width: '100%', padding: '7px 10px', background: 'transparent', border: `1px solid ${P.hairline2}`, borderRadius: P.r8, cursor: 'pointer', fontFamily: P.fontSans }}>
        ...
        <span style={{ fontSize: 10.5, color: P.inkMute, fontFamily: P.fontMono }}>{Math.min(a.tier + 1, 3)} / 3</span>
        <div style={{ flex: 1 }} />
...
window.FullOrderView = function FullOrderView({ order, m, onClose }) {
  ...
  const Meta = ({ k, v }) => <div><div style={{ fontSize: 9, color: P.inkMute, fontWeight: 700, letterSpacing: '.07em', textTransform: 'uppercase' }}>{k}</div><div style={{ fontSize: 12.5, fontWeight: 600, color: P.ink, marginTop: 2 }}>{v}</div></div>;
  return <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 200, background: P.scrim, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
    <div onClick={(e) => e.stopPropagation()} style={{ width: 'min(660px,96vw)', maxHeight: '90vh', display: 'flex', flexDirection: 'column', background: P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r16, boxShadow: P.shadowLg, overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '15px 20px', borderBottom: `1px solid ${P.hairline}` }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: 15.5, fontWeight: 800, color: P.ink, fontFamily: P.fontMono }}>#{o.id}</span><Pill kind="good" dot>{o.status}</Pill>{o.wm && <span style={{ fontSize: 9.5, fontWeight: 800, color: '#fff', background: '#1F5FC0', padding: '2px 8px', borderRadius: 99 }}>Weedmaps</span>}</div>
          <div style={{ fontSize: 11, color: P.inkDim, fontFamily: P.fontMono, marginTop: 2 }}>{o.date} · {m.name}</div>
        </div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ border: `1px solid ${P.hairline}`, borderRadius: P.r12, overflow: 'hidden' }}>
          <div style={{ padding: '8px 13px', background: P.surface2, borderBottom: `1px solid ${P.hairline}`, fontSize: 9.5, fontWeight: 800, letterSpacing: '.07em', textTransform: 'uppercase', color: P.inkMute }}>Items</div>
          {lines.map((l, i) => <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '10px 13px', borderTop: i ? `1px solid ${P.hairline}` : 'none' }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 10.5, color: P.inkMute, fontFamily: P.fontMono }}>{l.p.brand} · {l.p.cat}{l.p.wt ? ' · ' + l.p.wt : ''}</div>
            </div>
            <span style={{ fontSize: 11.5, color: P.inkDim, fontFamily: P.fontMono }}>×{l.qty}</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: P.ink, fontFamily: P.fontMono, minWidth: 62, textAlign: 'right' }}>{fmt.money(l.line)}</span>
          </div>)}
        </div>
        <div style={{ padding: '12px 14px', background: P.surface2, border: `1px solid ${P.hairline}`, borderRadius: P.r12 }}>
          ...
          <div style={{ marginTop: 6, fontSize: 11, color: P.inkDim, fontFamily: P.fontMono }}>Paid by {tender.toLowerCase()}</div>
        </div>
      </div>
        ...
        <div onClick={(e) => e.stopPropagation()} style={{ width: 'min(560px,96vw)', background: P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r16, boxShadow: P.shadowLg, overflow: 'hidden' }}>
          ...
          {p ? <div style={{ padding: 18 }}>
            <div style={{ display: 'flex', gap: 14, alignItems: 'center', marginBottom: 15 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 10.5, color: P.inkMute, fontFamily: P.fontMono }}>{p.brand}</div>
                <div style={{ fontSize: 17, fontWeight: 800, color: P.ink, letterSpacing: '-.01em', lineHeight: 1.2 }}>{p.name}</div>
                <div style={{ fontSize: 11, color: P.inkDim, fontFamily: P.fontMono, marginTop: 2 }}>{p.sku} · {p.cat}{p.wt ? ' · ' + p.wt : ''}</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                {p.was && <div style={{ fontSize: 12, color: P.inkFaint, textDecoration: 'line-through', fontFamily: P.fontMono }}>{fmt.money(p.was)}</div>}
                <div style={{ fontSize: 26, fontWeight: 800, color: p.was ? P.bad : P.ink, fontFamily: P.fontMono, lineHeight: 1.1 }}>{fmt.money(p.price)}</div>
                {p.was && <div style={{ fontSize: 10.5, fontWeight: 800, color: P.bad }}>SALE · save {fmt.money(p.was - p.price)}</div>}
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(96px,1fr))', gap: 9 }}>
              ...
              <div key={k} style={{ padding: '9px 11px', background: P.surface2, border: `1px solid ${P.hairline}`, borderRadius: P.r10 }}>
                  <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.07em', textTransform: 'uppercase', color: P.inkMute }}>{k}</div>
                  <div style={{ fontSize: 14, fontWeight: 700, color: c, fontFamily: P.fontMono, marginTop: 2 }}>{v}</div>
                </div>)}
            </div>
          ...
          <div style={{ maxHeight: 340, overflowY: 'auto' }}>
            {hits.map((x) => <button key={x.sku} onClick={() => setSel(x)} style={{ display: 'flex', alignItems: 'center', gap: 11, width: '100%', padding: '10px 16px', background: 'transparent', border: 'none', borderTop: `1px solid ${P.hairline}`, cursor: 'pointer', textAlign: 'left', fontFamily: P.fontSans }} onMouseEnter={(e) => e.currentTarget.style.background = P.surface2} onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 10.5, color: P.inkMute, fontFamily: P.fontMono }}>{x.brand} · {x.qty} on hand</div>
              </div>
              <span style={{ fontSize: 13.5, fontWeight: 700, color: x.was ? P.bad : P.ink, fontFamily: P.fontMono }}>{fmt.money(x.price)}</span>
            </button>)}
            {!q.trim() && <div style={{ padding: '26px 20px', textAlign: 'center', color: P.inkMute, fontSize: 12.5, lineHeight: 1.6 }}>Scan a barcode or start typing.<br /><span style={{ fontSize: 11 }}>Works from any screen — <b style={{ color: P.ink2 }}>F2</b> or <b style={{ color: P.ink2 }}>⌘K</b>.</span></div>}
