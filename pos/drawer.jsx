...
window.DrawerControls = function DrawerControls() {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 2 }}>
      ...
      style={{ display: 'inline-flex', alignItems: 'center', gap: 7, height: 38, padding: '0 12px', background: P.goodSoft, border: `1px solid ${P.good}`, borderRadius: P.r10, color: P.good, cursor: 'pointer', fontFamily: P.fontSans, fontSize: 12, fontWeight: 700, whiteSpace: 'nowrap' }}>
      ...
      style={{ display: 'inline-flex', alignItems: 'center', gap: 7, height: 38, padding: '0 12px', background: P.surface2, border: `1px solid ${P.hairline2}`, borderRadius: P.r10, color: P.ink2, cursor: 'pointer', fontFamily: P.fontSans, fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap' }}>
        ...
        {open && last && <>
          ...
          <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, width: 296, background: P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r14, boxShadow: P.shadowLg, padding: 14, zIndex: 41 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: P.ink }}>Last receipt</span>
              <div style={{ flex: 1 }} />
              <span style={{ fontSize: 10.5, color: P.inkMute, fontFamily: P.fontMono }}>{last.id}</span>
            </div>
            <div style={{ background: P.surface2, border: `1px solid ${P.hairline}`, borderRadius: P.r10, padding: '10px 12px', marginBottom: 12, display: 'flex', flexDirection: 'column', gap: 5 }}>
              ...
              <div style={{ fontSize: 10.5, color: P.inkMute, fontFamily: P.fontMono }}>{last.method === 'cash' ? 'Cash' : last.method === 'card' ? 'Card' : 'Split'}{last.cardCharged > 0 ? ` · card ${money(last.cardCharged)}` : ''}{last.change > 0 ? ` · change ${money(last.change)}` : ''}</div>
            </div>
  ...
  return (
    <div style={{ position: 'fixed', top: 70, left: '50%', transform: 'translateX(-50%)', zIndex: 90, display: 'flex', alignItems: 'center', gap: 13, padding: '12px 16px', background: P.ink, color: '#fff', borderRadius: P.r14, boxShadow: P.shadowLg, animation: 'fade .18s ease', maxWidth: 'calc(100vw - 40px)' }}>
      ...
      <div style={{ lineHeight: 1.25 }}>
        <div style={{ fontSize: 13, fontWeight: 700 }}>Cash drawer open</div>
        <div style={{ fontSize: 11, color: 'rgba(255,255,255,.6)', fontFamily: P.fontMono }}>{d.reason} · close the drawer to continue</div>
      </div>
      <button onClick={() => window.POS.closeDrawer()} style={{ marginLeft: 6, padding: '7px 13px', background: 'rgba(255,255,255,.12)', color: '#fff', border: 'none', borderRadius: P.r999, fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: P.fontSans, whiteSpace: 'nowrap' }}>Drawer closed</button>
    </div>);
...
function CountGrid({ counts, setCounts }) {
  ...
  return <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
    {DENOMS.map(([lbl, d]) => {const q = counts[d] || 0;return <div key={d} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', background: P.surface2, border: `1px solid ${P.hairline}`, borderRadius: P.r10 }}>
      <span style={{ width: 38, fontSize: 12, fontWeight: 700, color: P.ink2, fontFamily: P.fontMono }}>{lbl}</span>
      ...
      <span style={{ marginLeft: 'auto', fontSize: 12, fontWeight: 600, color: q ? P.ink : P.inkFaint, fontFamily: P.fontMono }}>{money(q * d)}</span>
    </div>;})}
  </div>;
}
  ...
  footer={<><PBtn variant="accent" size="md" icon="check" full disabled={!started} onClick={() => {window.POS.openRegister(total);onClose();}}>{matches ? 'Open register · ' + money(total) : started ? 'Open anyway · ' + (diff < 0 ? 'short ' : 'over ') + money(Math.abs(diff)) : 'Count the drawer'}</PBtn><PBtn variant="secondary" size="md" onClick={onClose}>Cancel</PBtn></>}>
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: P.accentSoft, border: `1px solid ${P.accentBorder}`, borderRadius: P.r12, marginBottom: 13 }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 9.5, fontWeight: 800, letterSpacing: '.07em', textTransform: 'uppercase', color: P.mode === 'dark' ? P.accent : '#7A5A00' }}>Every drawer in this store starts with</div>
        <div style={{ fontSize: 22, fontWeight: 800, color: P.ink, fontFamily: P.fontMono, lineHeight: 1.15 }}>{money(required)}</div>
      </div>
      <span style={{ fontSize: 10.5, color: P.inkDim, textAlign: 'right', lineHeight: 1.4, flex: '0 0 auto', maxWidth: 128 }}>Set by a manager in Settings → Cash Drawer</span>
    </div>
    <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: P.inkMute, marginBottom: 8 }}>Count what is in the drawer</div>
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 14, padding: '11px 13px', background: P.ink, color: '#fff', borderRadius: P.r12 }}>
      <div><div style={{ fontSize: 12.5, fontWeight: 600 }}>Counted {money(total)}</div><div style={{ fontSize: 10.5, color: 'rgba(255,255,255,.6)', fontFamily: P.fontMono }}>required {money(required)}</div></div>
      {started ?
        ...
        <span style={{ fontSize: 12, fontWeight: 600, fontFamily: P.fontMono, color: 'rgba(255,255,255,.45)' }}>not counted</span>}
    </div>
    <div style={{ fontSize: 11, color: c, fontWeight: 600, marginTop: 8, textAlign: 'center' }}>
...
function CloseRegisterModal({ onClose }) {
  ...
  footer={<><PBtn variant="accent" size="md" icon="check" full disabled={counted <= 0} onClick={() => {window.POS.closeRegister(counted);onClose();}}>Close &amp; reconcile</PBtn><PBtn variant="secondary" size="md" onClick={onClose}>Cancel</PBtn></>}>
    ...
    <div style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: P.inkMute, marginBottom: 8 }}>Counted</div>
    <CountGrid counts={counts} setCounts={setCounts} />
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 14, padding: '11px 13px', background: P.ink, color: '#fff', borderRadius: P.r12 }}>
      <div><div style={{ fontSize: 12.5, fontWeight: 600 }}>Counted {money(counted)}</div><div style={{ fontSize: 10.5, color: 'rgba(255,255,255,.6)', fontFamily: P.fontMono }}>expected {money(expected)}</div></div>
      {started ?
    ...
    <div style={{ fontSize: 11, color: vc, fontWeight: 600, marginTop: 8, textAlign: 'center' }}>{!started ? 'Enter the counted notes and coins above to see the variance' : variance === 0 ? 'Balanced — no variance' : variance > 0 ? 'Over by ' + money(Math.abs(variance)) : 'Short by ' + money(Math.abs(variance))}</div>
  </RegModal>;
}
