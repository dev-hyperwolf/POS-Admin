  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 24, padding: '26px 24px 60px', maxWidth: 1180, margin: '0 auto', alignItems: 'start' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
        ...
        {/* trigger */}
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 11 }}><StepDot n={1} /><span style={{ fontSize: 13.5, fontWeight: 700, color: P.ink }}>What triggers this promotion?</span><span style={{ fontSize: 12.5, color: P.inkMute }}>Pick what the rule looks at.</span></div>
        ...
        {e &&
        <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 11 }}><StepDot n={2} /><span style={{ fontSize: 13.5, fontWeight: 700, color: P.ink }}>Build the rule</span><span style={{ fontSize: 12.5, color: P.inkMute }}>Reads like a sentence. Click any underlined value to change it.</span></div>
            <Card padding={0} style={{ overflow: 'hidden' }}>
              {/* WHEN */}
              <div style={{ padding: '20px 22px', fontSize: 16 }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7, marginRight: 8, verticalAlign: '2px' }}><span style={{ fontSize: 11.5, fontWeight: 700, letterSpacing: '.12em', color: ec, fontFamily: P.fontMono, background: ec + (P.mode === 'dark' ? '22' : '14'), padding: '3px 9px', borderRadius: 6 }}>WHEN</span></span>
                <span style={{ fontWeight: 700, color: P.ink }}>{subj}</span>
                {rule.conditions.length === 0 && <span style={{ color: P.inkMute }}> …</span>}
                {rule.conditions.map((c, i) =>
              <span key={i}>
                    {i > 0 && <button onClick={() => setRule({ combiner: rule.combiner === 'AND' ? 'OR' : 'AND' })} style={{ margin: '0 7px', padding: '2px 9px', fontSize: 11.5, fontWeight: 700, letterSpacing: '.06em', color: P.mode === 'dark' ? P.accent : '#8A6200', background: P.accentSoft, border: 'none', borderRadius: 6, cursor: 'pointer', fontFamily: P.fontMono, verticalAlign: '2px' }} title="Toggle AND / OR">{rule.combiner}</button>}
                    {i === 0 && <span> </span>}
                ...
                <span style={{ display: 'inline-block', marginLeft: 8 }}>
                  ...
                  {addOpen && <AnchoredPopover anchorRef={addRef} onClose={() => setAddOpen(false)} width={580}>
                    <div style={{ padding: 18, overflowY: 'auto' }}>
                      <div style={{ fontSize: 12.5, color: P.inkDim, marginBottom: 14 }}>Pick a condition for <b style={{ color: P.ink }}>{e.label}</b>. Each shows what it does.</div>
              ...
              <div style={{ padding: '18px 22px', borderTop: `1px solid ${P.hairline}`, background: P.surface2 }}>
                <div style={{ marginBottom: rule.reward.id ? 12 : 0 }}>
                  <span style={{ fontSize: 11.5, fontWeight: 700, letterSpacing: '.12em', color: P.mode === 'dark' ? P.accent : '#8A6200', fontFamily: P.fontMono, background: P.accentSoft, padding: '3px 9px', borderRadius: 6, marginRight: 10 }}>THEN</span>
                  <span style={{ fontSize: 13.5, color: P.inkDim }}>we reward the customer with…</span>
                </div>
                {rule.reward.id ?
      ...
      <div style={{ position: 'sticky', top: 78, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <Card padding={16}>
          ...
          <ul style={{ margin: 0, paddingLeft: 16, fontSize: 12.5, lineHeight: 1.6, color: P.inkDim }}>
