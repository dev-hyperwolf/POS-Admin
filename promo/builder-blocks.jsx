  ...
  return (
    <div style={{ padding: '22px 24px 40px', maxWidth: 1180, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 11.5, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: P.inkMute, fontFamily: P.fontMono, marginRight: 4 }}>Trigger</span>
        {ENTITIES.map((en) => {const a = rule.entity === en.id;const c = toneColor(P, en.tone);return (
            <button key={en.id} onClick={() => pickEntity(en.id)} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '8px 14px', background: a ? c : P.surface, color: a ? '#fff' : P.ink2, border: `1.5px solid ${a ? c : P.hairline2}`, borderRadius: 99, cursor: 'pointer', fontSize: 13.5, fontWeight: 600, fontFamily: P.fontSans, transition: 'all .12s' }}><Icon name={en.icon} size={15} stroke={1.9} />{en.label}</button>);})}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 60px 1fr', gap: 0, alignItems: 'start' }}>
        ...
        <div style={{ background: P.surface2, border: `1px solid ${P.hairline2}`, borderRadius: P.r16, padding: 16, minHeight: 220 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}><span style={{ fontSize: 11.5, fontWeight: 700, letterSpacing: '.14em', color: P.info, fontFamily: P.fontMono, background: P.infoSoft, padding: '4px 10px', borderRadius: 6 }}>IF</span><span style={{ fontSize: 12.5, color: P.inkDim }}>{e ? `all of these about ${subj}` : 'pick a trigger to start'}</span></div>
          ...
          {e && <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {rule.conditions.map((c, i) => {const cd = RULE.cond(rule.entity, rule.group, c.condId);return (
                <div key={i}>
                {i > 0 && <div style={{ display: 'flex', justifyContent: 'center', margin: '2px 0' }}><button onClick={() => setRule({ combiner: rule.combiner === 'AND' ? 'OR' : 'AND' })} style={{ padding: '2px 12px', fontSize: 11.5, fontWeight: 700, letterSpacing: '.08em', color: P.mode === 'dark' ? P.accent : '#8A6200', background: P.accentSoft, border: 'none', borderRadius: 99, cursor: 'pointer', fontFamily: P.fontMono }}>{rule.combiner}</button></div>}
                <div draggable onDragStart={() => setDrag(i)} onDragOver={(ev) => ev.preventDefault()} onDrop={() => {reorder(drag, i);setDrag(null);}} style={{ background: P.surface, border: `1px solid ${P.hairline2}`, borderLeft: `3px solid ${ec}`, borderRadius: P.r12, padding: '13px 14px', opacity: drag === i ? .4 : 1, boxShadow: P.shadowSm }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    ...
                    <span style={{ fontSize: 11.5, fontWeight: 600, letterSpacing: '.05em', textTransform: 'uppercase', color: P.inkMute, flex: 1 }}>{cd?.label}</span>
                    <IconBtn icon="trash" size={14} onClick={() => removeCond(i)} />
                  </div>
                  <div style={{ fontSize: 13.5 }}><b style={{ color: P.ink }}>{subj} </b><ClauseInline entityId={rule.entity} groupId={rule.group} cond={c} onChange={(v) => updateCond(i, v)} /></div>
                </div>
              </div>);})}
            {/* add block */}
        ...
        <div style={{ background: P.surface2, border: `1px solid ${P.hairline2}`, borderRadius: P.r16, padding: 16, minHeight: 220 }}>
          ...
          {rule.reward.id ? (() => {const r = RULE.reward(rule.reward.id);return (
              ...
              <div style={{ fontSize: 16, lineHeight: 1.7 }}><RewardInline reward={rule.reward} onChange={(v) => setRule({ reward: { ...rule.reward, values: v } })} /></div>
            </div>);})() :
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {REWARDS.map((r) => <button key={r.id} onClick={() => setRule({ reward: window.rewardDefaults(r.id) })} style={{ display: 'flex', alignItems: 'center', gap: 11, textAlign: 'left', padding: '12px 14px', background: P.surface, border: `1.5px dashed ${P.hairline3}`, borderRadius: P.r12, cursor: 'pointer', fontFamily: P.fontSans }}>
                  ...
                  <span><span style={{ fontSize: 12.5, fontWeight: 600, color: P.ink, display: 'block', marginBottom: 2 }}>{r.label}</span><span style={{ fontSize: 11.5, color: P.inkDim }}>{r.hint}</span></span>
                </button>)}
              </div>}
        </div>
      </div>
...
function WmChannelCard({ draft }) {
  ...
  return <Card padding={0}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '13px 18px', borderBottom: `1px solid ${P.hairline}` }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13.5, fontWeight: 700, color: P.ink }}>Weedmaps channel</div><div style={{ fontSize: 11.5, color: P.inkDim }}>Params fetched from Weedmaps for this promotion · updated 2m ago</div></div>
    ...
    <div style={{ padding: 18, display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: '12px 18px' }}>
      {params.map(([k, v]) => <div key={k} style={{ display: 'flex', flexDirection: 'column', gap: 3 }}><span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: P.inkMute }}>{k}</span><span style={{ fontSize: 12.5, fontWeight: 600, color: P.ink, fontFamily: P.fontMono }}>{v}</span></div>)}
    </div>
    <div style={{ padding: '0 18px 16px', fontSize: 11.5, color: P.inkDim, lineHeight: 1.5 }}>These come from the Weedmaps API for this promotion. Pricing &amp; availability flow <b style={{ color: P.ink2 }}>POS → Weedmaps</b>.</div>
  </Card>;
}
  ...
  return (
    <div style={{ padding: '22px 24px 48px', maxWidth: 960, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '13px 15px', background: P.mode === 'dark' ? 'rgba(31,95,192,.10)' : '#eaf1fb', border: '1px solid #1F5FC0', borderRadius: P.r14 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 13.5, fontWeight: 700, color: P.ink }}>Read-only — synced from Weedmaps</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 21, fontWeight: 800, letterSpacing: '-.02em', color: P.ink }}>{val(p.name)}</div>
          <div style={{ fontSize: 11.5, color: P.inkMute, fontFamily: P.fontMono }}>{val(p.wm_id)} · created via {val(p.created_source)} · last synced {val(p.last_synced)}</div>
        </div>
      ...
      <div style={{ background: P.mode === 'dark' ? P.surface2 : '#15140F', color: P.mode === 'dark' ? P.ink : '#F4F2EC', borderRadius: P.r14, padding: '18px 20px', border: `1px solid ${P.mode === 'dark' ? P.hairline2 : 'transparent'}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 11 }}>
          ...
          <span style={{ fontSize: 11.5, fontWeight: 700, letterSpacing: '.14em', textTransform: 'uppercase', color: '#6ea8f0', fontFamily: P.fontMono }}>How it reads</span>
        </div>
        ...
        <div style={{ marginTop: 13, paddingTop: 12, borderTop: `1px solid ${P.mode === 'dark' ? P.hairline : 'rgba(255,255,255,.12)'}`, fontSize: 12.5, opacity: .62 }}>{p.stackable ? 'Stacks with other offers' : 'Does not stack'} · priority {val(p.priority)} · {arr(p.wm_menu_ids) !== '—' ? `${(p.wm_menu_ids || []).length} WM menu(s)` : 'all menus'}</div>
      </div>

      {/* WM → Hyperwolf mapping */}
      <Card padding={0}>
        <div style={{ padding: '12px 16px', borderBottom: `1px solid ${P.hairline}` }}><div style={{ fontSize: 13.5, fontWeight: 700, color: P.ink }}>Weedmaps → Hyperwolf mapping</div><div style={{ fontSize: 11.5, color: P.inkDim, marginTop: 1 }}>How each Weedmaps field translates into our promotion schema.</div></div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', minWidth: 560, borderCollapse: 'collapse', fontSize: 12.5 }}>
            <thead><tr style={{ background: P.surface2 }}>{['Weedmaps field', 'Value', 'Our field', 'Maps to'].map((h, i) => <th key={i} style={{ textAlign: 'left', padding: '8px 14px', fontSize: 10, fontWeight: 700, letterSpacing: '.06em', textTransform: 'uppercase', color: P.inkDim, borderBottom: `1px solid ${P.hairline2}` }}>{h}</th>)}</tr></thead>
