  ...
  return (
    <div style={{ background: P.mode === 'dark' ? P.surface2 : '#15140F', color: P.mode === 'dark' ? P.ink : '#F4F2EC', borderRadius: P.r14, padding: compact ? '16px 18px' : '20px 22px', border: `1px solid ${P.mode === 'dark' ? P.hairline2 : 'transparent'}` }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
        ...
        <span style={{ fontSize: 10.5, fontWeight: 700, letterSpacing: '.14em', textTransform: 'uppercase', color: P.accent, fontFamily: P.fontMono }}>In plain English</span>
      </div>
      {ready ? <>
      ...
      </> : <div style={{ fontSize: 14.5, lineHeight: 1.6, opacity: .5 }}>Your promotion will read as a sentence here as you build it — pick a trigger, a condition, and a reward.</div>}
    </div>);
};
...
function CodeGen({ draft, set }) {
  ...
  return <span style={{ position: 'relative', display: 'inline-flex' }}>
    <button onClick={() => setOpen((o) => !o)} style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: 11, fontWeight: 600, color: P.info, background: 'none', border: 'none', cursor: 'pointer', fontFamily: P.fontSans, whiteSpace: 'nowrap' }}>Generate<Icon name="chevron-down" size={11} stroke={2.2} /></button>
    {open && <>
      ...
      <div style={{ position: 'absolute', top: 'calc(100% + 6px)', right: 0, width: 250, background: P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r12, boxShadow: P.shadowLg, zIndex: 61, padding: 12 }}>
        ...
        <div style={{ fontSize: 9.5, fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', color: P.inkMute, marginBottom: 7 }}>Bulk unique codes</div>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <input value={qty} onChange={(e) => setQty(e.target.value.replace(/[^0-9]/g, ''))} inputMode="numeric" placeholder="Custom…" style={{ flex: 1, minWidth: 0, padding: '7px 10px', border: `1px solid ${P.hairline2}`, borderRadius: P.r8, background: P.field || P.surface, color: P.ink, fontSize: 12, fontFamily: P.fontMono, outline: 'none', boxSizing: 'border-box' }} />
          <button onClick={() => qty && bulk(+qty)} disabled={!qty} style={{ fontSize: 12, fontWeight: 700, color: qty ? P.accentInk : P.inkFaint, background: qty ? P.accent : P.surface3, border: 'none', borderRadius: P.r8, padding: '7px 12px', cursor: qty ? 'pointer' : 'default', fontFamily: P.fontSans }}>Generate</button>
        </div>
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr auto', gap: 14, alignItems: 'end' }}>
      ...
      <div>
        ...
        {draft.codeBatch ? <div style={{ fontSize: 10.5, fontWeight: 600, color: P.info, marginTop: 5, fontFamily: P.fontMono }}>{draft.codeBatch.toLocaleString()} unique codes queued</div> : null}
      </div>
      <div>
...
window.ScheduleLimits = function ScheduleLimits({ draft, set }) {
  const P = useP();
  const Row = ({ label, children }) => <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>{children}<span style={{ fontSize: 13, color: P.ink2, fontWeight: 500 }}>{label}</span></div>;
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {!draft.publishNow && <div style={{ paddingLeft: 2 }}><Eyebrow style={{ marginBottom: 6, fontSize: 9.5 }}>Publish date</Eyebrow><Field icon="calendar" value={draft.publishDate} onChange={(e) => set({ publishDate: e.target.value })} size="sm" /></div>}
          <Row label="Set an expiry date"><Switch on={draft.expiry} onChange={(v) => set({ expiry: v })} /></Row>
          {draft.expiry && <div style={{ paddingLeft: 2 }}><Eyebrow style={{ marginBottom: 6, fontSize: 9.5 }}>Expiry date</Eyebrow><Field icon="calendar" value={draft.expiryDate} onChange={(e) => set({ expiryDate: e.target.value })} size="sm" /></div>}
        </div>
      </div>
      <div>
        <Eyebrow style={{ marginBottom: 12 }}>Usage limits</Eyebrow>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div><Eyebrow style={{ marginBottom: 6, fontSize: 9.5 }}>Total uses (all customers)</Eyebrow><Field placeholder="Unlimited" value={draft.totalLimit} onChange={(e) => set({ totalLimit: e.target.value })} mono size="sm" /></div>
          <div><Eyebrow style={{ marginBottom: 6, fontSize: 9.5 }}>Uses per customer</Eyebrow><Field placeholder="Unlimited" value={draft.userLimit} onChange={(e) => set({ userLimit: e.target.value })} mono size="sm" /></div>
        </div>
      </div>
    </div>);
};
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${columns},1fr)`, gap: 12 }}>
      {ENTITIES.map((e) => {const a = value === e.id;const c = toneColor(P, e.tone);return (
          ...
          <span style={{ fontSize: 14, fontWeight: 700, color: P.ink }}>{e.label}</span>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {REWARDS.map((r) => {const a = value === r.id;return (
          ...
          <span><span style={{ fontSize: 13.5, fontWeight: 600, color: P.ink, display: 'block', marginBottom: 3 }}>{r.label}</span><span style={{ fontSize: 12, color: P.inkDim, lineHeight: 1.4 }}>{r.hint}</span></span>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div key={g.id}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 9 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.08em', textTransform: 'uppercase', color: P.inkMute }}>{g.label}</span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {g.conditions.map((c) => {const used = activeCondIds.includes(c.id);return (
              <button key={c.id} disabled={used} onClick={() => onPick(g.id, c.id)} style={{ textAlign: 'left', padding: '11px 13px', background: used ? P.surface2 : P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r10, cursor: used ? 'default' : 'pointer', opacity: used ? .5 : 1, transition: 'all .12s' }} onMouseEnter={(ev) => !used && (ev.currentTarget.style.borderColor = P.hairline3)} onMouseLeave={(ev) => ev.currentTarget.style.borderColor = P.hairline2}>
                ...
                <span style={{ fontSize: 11, color: P.inkDim, lineHeight: 1.4 }}>{c.hint}</span>
              </button>);})}
          </div>
        </div>)}
    </div>);
...
window.BuilderTopBar = function BuilderTopBar({ mode, setMode, onCancel, onSave, draft }) {
  ...
  if (ro) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 24px', borderBottom: `1px solid ${P.hairline2}`, background: P.surface, position: 'sticky', top: 0, zIndex: 25 }}>
        ...
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 800, color: '#fff', background: '#1F5FC0', padding: '3px 9px', borderRadius: 99 }}><span style={{ width: 6, height: 6, borderRadius: 2, background: '#fff' }} />Weedmaps promotion</span>
        <span style={{ fontSize: 12, color: P.inkDim }}>{draft.name || 'Untitled'}</span>
        <div style={{ flex: 1 }} />
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 24px', borderBottom: `1px solid ${P.hairline2}`, background: P.surface, position: 'sticky', top: 0, zIndex: 25 }}>
      ...
      <div style={{ width: 1, height: 22, background: P.hairline2 }} />
      <span style={{ fontSize: 12, fontWeight: 700, color: P.ink }}>New promotion</span>
      <div style={{ flex: 1 }} />
      <span style={{ fontSize: 12, color: P.inkDim }}>{draft.name || 'Untitled promotion'}</span>
