      ...
      <div style={{ flexBasis: '100%', marginTop: 8, borderRadius: 8, border: `1px solid ${P.hairline2}`, background: P.surface, padding: 8, boxShadow: P.shadowMd }}>
        ...
        <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
          ...
          {(brandFilter.size + categoryFilter.size) > 0 && (
            <button onClick={() => { setBrandFilter(new Set()); setCategoryFilter(new Set()); }} style={{ marginLeft: 'auto', height: 20, padding: '0 8px', fontSize: 11.5, color: P.inkMute, background: 'none', border: 'none', cursor: 'pointer', fontFamily: P.fontSans }}>Clear all</button>)}
        </div>
        <ul role="listbox" style={{ listStyle: 'none', margin: '8px 0 0', padding: 0, maxHeight: 192, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 2 }}>
          {filtered.length === 0
            ? <li style={{ fontSize: 12.5, color: P.inkMute, padding: '6px 8px' }}>No products match.</li>
            : filtered.map((p) => (
              <li key={p.id}>
                <button role="option" onClick={() => { store.setLineDecision(lineId, { kind: 'picked', productId: p.id }); onClose(); }}
                  ...
                  {p.score > 0 && <span style={{ fontSize: 11.5, color: P.inkMute, fontFamily: P.fontMono }}>{Math.round(p.score * 100)}%</span>}
                </button>
              </li>))}
        </ul>
        <div style={{ marginTop: 4, display: 'flex', justifyContent: 'flex-end' }}>
          <button onClick={onClose} style={{ height: 24, padding: '0 8px', fontSize: 11.5, color: P.inkMute, background: 'none', border: 'none', cursor: 'pointer', fontFamily: P.fontSans }}>Cancel</button>
        </div>
      </div>);
  }
    ...
    return (
      <Sheet open={open} onClose={onClose} width={480}>
        <div style={{ padding: 20, borderBottom: `1px solid ${P.hairline2}`, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11.5, textTransform: 'uppercase', letterSpacing: '.06em', color: HD.hueColor(P, 'violet') }}>
              <Icon name="sparkle" size={12} stroke={2} />Hyperdrive Helper
            </div>
            <h2 style={{ margin: '4px 0 0', fontSize: 16, fontWeight: 600, color: P.ink }}>About {invoice.invoiceNumber}</h2>
          </div>
          <IconBtn icon="x" size={16} onClick={onClose} style={{ width: 30, height: 30, margin: -4 }} />
        </div>
        <div style={{ flex: 1, overflow: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ fontSize: 13.5, color: P.ink2, lineHeight: 1.5 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {need.map(({ line, match }) => (
                  <div key={line.id} style={{ padding: 10, borderRadius: 8, border: `1px solid ${P.hairline2}`, background: P.surface3 }}>
                    ...
                    <div style={{ fontSize: 11.5, color: P.inkMute, marginTop: 2, fontFamily: P.fontMono }}>{line.sku} · ×{line.qty} · best match {Math.round(match.confidence * 100)}%</div>
                  </div>))}
              </div>
            </div>)}
          <div style={{ fontSize: 12.5, color: P.inkMute, lineHeight: 1.5 }}>
    ...
    if (mapped) {
      return (
        <div role="status" style={{ borderRadius: 8, border: `1px solid ${P.accentBorder}`, background: P.accentSoft, padding: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ fontSize: 13.5, color: P.ink }}>
            <strong>{matches.length} batches</strong> mapped from this invoice.{' '}
            <button onClick={() => { location.hash = '#/batches'; }} style={{ background: 'none', border: 'none', padding: 0, color: accentInk, textDecoration: 'underline', textUnderlineOffset: 2, cursor: 'pointer', fontSize: 13.5, fontFamily: P.fontSans }}>View on Batches →</button>
          </div>
        </div>);
    }
