...
;(function () {
        ...
        {batches.map((b) => (
          <button key={b.id} onClick={() => navigate('#/batches')} style={{ display: 'grid', gridTemplateColumns: INV_GRID, width: '100%', textAlign: 'left', border: 'none', background: 'transparent', borderBottom: `1px solid ${P.hairline}`, cursor: 'pointer', fontFamily: P.fontSans }}
            ...
            <div style={{ paddingLeft: 28, paddingRight: 12, paddingTop: 8, paddingBottom: 8 }}>
              <div style={{ fontFamily: P.fontMono, fontSize: 12.5, color: P.ink2 }}>{b.metrcPackageId.slice(0, 12)}…{b.metrcPackageId.slice(-4)}</div>
              <div style={{ fontSize: 11.5, color: P.inkMute, marginTop: 2 }}>{b.packageDate ? `pkg ${HD.formatDate(b.packageDate)}` : 'pkg —'}</div>
            </div>
            ...
            <div style={{ padding: '8px 12px', textAlign: 'right', fontFamily: P.fontMono, fontSize: 12.5, color: P.ink }}>{b.qty} @ {HD.formatCurrency(b.unitValue)}</div>
            <div style={{ padding: '8px 12px', display: 'flex', alignItems: 'center' }}><ExpiryLabel days={daysToExpiry(b)} /></div>
            <div style={{ padding: '8px 12px', textAlign: 'right', fontFamily: P.fontMono, fontSize: 12.5, color: P.ink }}>{HD.formatCurrency(b.qty * b.unitValue, { showCents: false })}</div>
          </button>))}
      ...
      <React.Fragment>
        <button onClick={() => setOpen((v) => !v)} style={{ display: 'grid', gridTemplateColumns: INV_GRID, width: '100%', textAlign: 'left', borderBottom: `1px solid ${P.hairline}`, background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: P.fontSans }}
          ...
          <div style={{ padding: '12px' }}>
            <div style={{ fontSize: 13.5, color: P.ink }}>{group.productName}</div>
            <div style={{ fontSize: 11.5, color: P.inkMute, marginTop: 2 }}>{group.vendorName}</div>
          </div>
          <div style={{ padding: '12px', fontSize: 13.5, color: P.ink2 }}>{group.batches.length}</div>
          <div style={{ padding: '12px', textAlign: 'right', fontFamily: P.fontMono, fontSize: 13.5, color: P.ink }}>{group.totalUnits.toLocaleString()}</div>
          <div style={{ padding: '12px' }}><ExpiryLabel days={group.soonestExpiryDays} /></div>
          <div style={{ padding: '12px', textAlign: 'right', fontFamily: P.fontMono, fontSize: 13.5, color: P.ink }}>{HD.formatCurrency(group.totalValue, { showCents: false })}</div>
        </button>
  ...
  window.ScreenInventory = function ScreenInventory({ navigate }) {
    ...
    const head = (label, align) => <div style={{ padding: '8px 12px', fontSize: 11.5, textTransform: 'uppercase', letterSpacing: '.06em', color: P.inkMute, textAlign: align }}>{label}</div>;

    return (
      <div style={{ padding: 20, maxWidth: 1400, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 30, fontWeight: 600, letterSpacing: '-.02em', color: P.ink, lineHeight: 1.1 }}>Inventory</h1>
            <p style={{ margin: '4px 0 0', fontSize: 13.5, color: P.inkDim }}>Approved-for-sale batches. Group by product (default) or flip to a flat batch list.</p>
          </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {[['velocity', 'Velocity'], ['age', 'Age'], ['value', '$ value'], ['name', 'Name']].map(([s, label]) => (
              <button key={s} onClick={() => setSort(s)} style={{ fontSize: 12.5, padding: '0 12px', height: 32, borderRadius: 8, cursor: 'pointer', fontFamily: P.fontSans,
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8 }}>
            ...
            {[['all', 'All locations'], ['foh', 'Front of house'], ['boh', 'Back of house']].map(([id, label]) => (
              <button key={id} aria-pressed={location === id} onClick={() => setLocation(id)}
                style={{ fontSize: 12.5, padding: '0 10px', height: 28, borderRadius: 6, border: 'none', cursor: 'pointer', fontFamily: P.fontSans,
                  background: location === id ? P.ink : 'transparent', color: location === id ? P.surface : P.inkDim }}>{label}</button>))}
          </div>
          ...
          {hasActiveFilters && <button onClick={clearAll} style={{ background: 'none', border: 'none', padding: 0, marginLeft: 4, fontSize: 12.5, color: P.inkDim, textDecoration: 'underline', textUnderlineOffset: 2, cursor: 'pointer', fontFamily: P.fontSans }}>Clear all</button>}
        </div>
    ...
    return (
      <Card padding={0} hover onClick={() => navigate(`#/products/${product.id}`)} style={{ overflow: 'hidden', height: '100%', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 8, flex: 1 }}>
          ...
          <div style={{ marginTop: 'auto', paddingTop: 8, borderTop: `1px solid ${P.hairline}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 12.5 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', padding: 20, gap: 20 }}>
        <header style={{ display: 'flex', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 260 }}>
            <h1 style={{ margin: 0, fontSize: 21, fontWeight: 600, letterSpacing: '-.02em', color: P.ink }}>Products</h1>
            <p style={{ margin: '4px 0 0', fontSize: 13.5, color: P.inkMute }}>Customer-facing SKU families. Each card wraps the batches sitting underneath it.</p>
          </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {visible.length === 0
            ? <Card padding={40} style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 13.5, color: P.ink, marginTop: 8 }}>No products match.</div>
              <div style={{ fontSize: 12.5, color: P.inkMute, marginTop: 4 }}>Adjust your filters or clear the search box.</div>
            </Card>
