      ...
      <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 20 }}>
        <header style={{ display: 'flex', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 280 }}>
            <button onClick={() => navigate('#/batches')} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', padding: 0, marginBottom: 6, fontSize: 12, color: P.inkMute, cursor: 'pointer', fontFamily: P.fontSans }}>
              <Icon name="arrow-left" size={12} stroke={2} />Back to active pipeline
            </button>
            <h1 style={{ margin: 0, fontSize: 22, fontWeight: 600, letterSpacing: '-.02em', color: P.ink }}>Batch archive</h1>
            <p style={{ margin: '2px 0 0', fontSize: 13, color: P.inkMute, maxWidth: 620 }}>Approved-for-sale, destroyed, resolved recalls, and shelf-ready batches that have aged out of the active board.</p>
          </div>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 20 }}>
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2, justifyContent: 'flex-end' }}>
                ...
                <span style={{ fontSize: 13, color: P.ink }}>{entityMeta?.short ?? entity.toUpperCase()}</span>
              </div>
            </div>
              ...
              <HDTable>
                ...
                <tbody>
                  {sorted.map((b) => (
                    <TR key={b.id}>
                      <TD>
                        <div style={{ fontSize: 13, color: P.ink, maxWidth: 280, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.productName}</div>
                        <div style={{ fontSize: 11, color: P.inkMute, maxWidth: 280, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.brand}</div>
                      </TD>
                      <TD mono style={{ fontSize: 12, color: P.ink2 }}>{b.sku}</TD>
                      <TD><HDPill tone={HD.batchStatusTone(b.status)} label={HD.BATCH_STATUS_LABEL[b.status]} icon={false} size="sm" /></TD>
                      <TD style={{ fontSize: 12, color: P.ink2, whiteSpace: 'nowrap' }}>{HD.formatDate(HD.archivedAt(b))}</TD>
                      <TD align="right" mono>{b.qty.toLocaleString()}</TD>
                      <TD align="right" mono>{HD.formatCurrency(b.qty * b.unitValue, { showCents: false })}</TD>
                      <TD align="right"><span title={b.metrcPackageId} style={{ fontFamily: P.fontMono, fontSize: 11, color: P.inkMute }}>{HD.uidShort(b.metrcPackageId, 'metrc')}</span></TD>
                    </TR>))}
                </tbody>
              </HDTable>
            </div>}
  ...
  function HeaderColumn({ batch, accent }) {
    ...
    return (
      <div style={{ padding: 16 }}>
        ...
        <div style={{ fontSize: 13, color: P.ink, lineHeight: 1.35 }}>{batch.productName}</div>
  ...
  function ComparisonRow({ field, left, right, match, critical }) {
    ...
    return (
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 180px 1fr', borderBottom: `1px solid ${P.hairline}`, background: match ? 'transparent' : bad.bg }}>
        <div style={{ padding: '10px 16px', fontFamily: P.fontMono, fontSize: 12, color: P.ink }}>{displayLeft}</div>
        <div style={{ padding: '10px 12px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, borderLeft: `1px solid ${P.hairline2}`, borderRight: `1px solid ${P.hairline2}` }}>
          ...
          <span style={{ fontSize: 11, color: match ? ok.fg : bad.fg }}>
            {field}{!match && critical && <span style={{ marginLeft: 4, fontSize: 9, fontWeight: 700, textTransform: 'uppercase' }}>blocking</span>}
          </span>
        </div>
        <div style={{ padding: '10px 16px', fontFamily: P.fontMono, fontSize: 12, color: P.ink }}>{displayRight}</div>
      </div>);
  }
  ...
  function MergeWizard({ left, right, navigate }) {
    ...
    return (
      <div style={{ padding: 20, maxWidth: 960, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
        <button onClick={() => navigate('#/batches')} style={{ display: 'inline-flex', alignSelf: 'flex-start', alignItems: 'center', gap: 6, background: 'none', border: 'none', padding: 0, fontSize: 13, color: P.inkMute, cursor: 'pointer', fontFamily: P.fontSans }}>
          <Icon name="chevron-left" size={14} stroke={2} /> Back
        </button>
        <div>
          <MicroLabel>Merge packages</MicroLabel>
          <h1 style={{ margin: '2px 0 0', fontSize: 26, fontWeight: 600, letterSpacing: '-.02em', color: P.ink, lineHeight: 1.1 }}>Combine into one package</h1>
          <p style={{ margin: '4px 0 0', fontSize: 13, color: P.inkDim, maxWidth: 640 }}>Merging two packages replaces them with one new METRC package. Every critical field must match or the operation blocks.</p>
        </div>
        ...
        {blocked ? (
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, borderRadius: P.r12, border: `1px solid ${blockedTone.fg}80`, background: blockedTone.bg, padding: '12px 16px' }}>
            ...
            <div>
              <div style={{ fontSize: 14, color: blockedTone.fg, fontWeight: 500 }}>Merge blocked — {criticalMismatches.length} critical mismatch{criticalMismatches.length === 1 ? '' : 'es'}</div>
              <div style={{ fontSize: 12, color: P.inkMute, marginTop: 2 }}>{criticalMismatches.map(([k]) => FIELD_LABELS[k]).join(', ')} differ. These fields cannot be overridden.</div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, borderRadius: P.r12, border: `1px solid ${warnTone.fg}66`, background: warnTone.bg, padding: '12px 16px' }}>
            ...
            <div>
              <div style={{ fontSize: 13, color: warnTone.fg, fontWeight: 500 }}>{nonCriticalMismatches.length} non-critical mismatch{nonCriticalMismatches.length === 1 ? '' : 'es'} — you can override, but document why.</div>
              <div style={{ fontSize: 12, color: P.inkMute, marginTop: 2 }}>{nonCriticalMismatches.map(([k]) => FIELD_LABELS[k]).join(', ')} differ.</div>
            </div>
          </div>
        ) : (
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, borderRadius: P.r12, border: `1px solid ${okTone.fg}4d`, background: okTone.bg, padding: '12px 16px' }}>
            ...
            <div style={{ fontSize: 13, color: okTone.fg }}>All 7 fields match. This merge is eligible.</div>
          </div>
        )}
        ...
        {!blocked && nonCriticalMismatches.length > 0 && (
          <label style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: 12, borderRadius: P.r10, border: `1px solid ${warnTone.fg}4d`, background: warnTone.bg, cursor: 'pointer' }}>
            ...
            <span style={{ fontSize: 12, color: warnTone.fg }}>I acknowledge the non-critical mismatches above and confirm the merge is still appropriate.</span>
          </label>)}
          ...
          {selected ? (
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: 12, borderRadius: P.r10, background: P.bg2, border: `1px solid ${P.accentBorder}` }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 11, color: P.inkMute, marginTop: 4, fontFamily: P.fontMono }}>{selected.sku} · {selected.qty} units</div>
              </div>
            ...
            <React.Fragment>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                {candidates.length === 0 && <div style={{ padding: '16px 8px', textAlign: 'center', fontSize: 12, color: P.inkMute }}>No matching packages.</div>}
              </div>
            </React.Fragment>)}
        </div>
      </Card>);
  ...
  window.ScreenBatchMerge = function ScreenBatchMerge({ query, navigate }) {
    ...
    return (
      <div style={{ padding: 20, maxWidth: 960, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div>
          <MicroLabel>Merge packages</MicroLabel>
          <h1 style={{ margin: '2px 0 0', fontSize: 26, fontWeight: 600, letterSpacing: '-.02em', color: P.ink, lineHeight: 1.1 }}>Select two packages to merge</h1>
          <p style={{ margin: '4px 0 0', fontSize: 13, color: P.inkDim }}>Same-SKU batches on the floor are eligible. Eligibility is validated in the next step.</p>
        </div>
