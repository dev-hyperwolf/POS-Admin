  ...
  function ToggleChip({ active, icon, label, onClick }) {
    ...
    return (
      <button onClick={onClick} aria-pressed={active}
        style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 8px', borderRadius: 99, fontSize: 11.5, cursor: 'pointer', fontFamily: P.fontSans,
  ...
  function StageRow({ stage, index, onToggle, onRemove, onRename, onDragStart, onDragOver, onDrop, dragging }) {
    ...
    return (
      <li draggable onDragStart={onDragStart} onDragOver={onDragOver} onDrop={onDrop}
        ...
        <span style={{ fontSize: 11.5, fontFamily: P.fontMono, color: P.inkMute, width: 24 }}>{index + 1}.</span>
        ...
        {editing
          ? <input autoFocus value={stage.stageLabel} maxLength={24} onChange={(e) => onRename(stage.stageKey, e.target.value)}
            ...
            style={{ background: P.field, border: `1px solid ${P.fieldBorder}`, borderRadius: 6, padding: '2px 8px', fontSize: 13.5, color: P.ink, width: 160, fontFamily: P.fontSans, outline: 'none' }} />
          : <button onClick={() => setEditing(true)} style={{ fontSize: 13.5, color: P.ink, background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontFamily: P.fontSans }}>{stage.stageLabel}</button>}
    ...
    return (
      <div style={{ maxWidth: 960, margin: '0 auto', padding: 20 }}>
        <div style={{ marginBottom: 16, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
          <div>
            <button onClick={() => navigate('#/batches')} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'none', border: 'none', padding: 0, marginBottom: 6, fontSize: 12.5, color: P.inkMute, cursor: 'pointer', fontFamily: P.fontSans }}>
              <Icon name="arrow-left" size={12} stroke={2} /> Back to batches
            </button>
            <h1 style={{ margin: 0, fontSize: 30, fontWeight: 600, letterSpacing: '-.02em', color: P.ink }}>Pipeline config · {entityMeta.name}</h1>
            <p style={{ margin: '4px 0 0', fontSize: 13.5, color: P.inkDim, maxWidth: 680 }}>Drag to reorder. Incoming is fixed; Approved, Quarantined, Recalled and Destroyed are universal and not configurable here.</p>
          </div>
        ...
        <Card padding={16}>
          ...
          <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${P.hairline2}` }}>
            <button onClick={() => setAddOpen((v) => !v)} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', padding: 0, fontSize: 13.5, color: P.ink2, cursor: 'pointer', fontFamily: P.fontSans }}>
              <Icon name="plus" size={14} stroke={2.2} /> Add stage from catalog
            </button>
            {addOpen && (
              <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {availableToAdd.length === 0
                  ? <span style={{ fontSize: 12.5, color: P.inkMute, fontStyle: 'italic' }}>All workflow stages already in the pipeline.</span>
                  : availableToAdd.map((k) => (
                    <button key={k} onClick={() => addStage(k)} style={{ padding: '4px 10px', fontSize: 12.5, borderRadius: 8, border: `1px solid ${P.hairline2}`, background: P.surface2, color: P.ink2, cursor: 'pointer', fontFamily: P.fontSans }}>
        ...
        <Card padding={16} style={{ marginTop: 24 }}>
          <div>
            ...
            <p style={{ margin: '4px 0 0', fontSize: 12.5, color: P.inkDim }}>Specific masters add or skip stages (Phase 2 — pipeline.per_product_overrides_enabled).</p>
          </div>
  ...
  function CatalogIndex({ navigate }) {
    ...
    return (
      <div style={{ maxWidth: 960, margin: '0 auto', padding: 20 }}>
        ...
        <h1 style={{ margin: 0, fontSize: 30, fontWeight: 600, letterSpacing: '-.02em', color: P.ink }}>Catalog · Master products</h1>
        <p style={{ margin: '4px 0 0', fontSize: 13.5, color: P.inkDim, maxWidth: 680 }}>The packaging toggle drives the pipeline resolver. Products flagged tamper-evident auto-skip the Sealing stage at The Highest Craft.</p>
              ...
              {rows.map((r) => (
                <TR key={r.masterProductId}>
                  <TD>
                    ...
                    <span style={{ display: 'block', fontFamily: P.fontMono, fontSize: 11.5, color: P.inkMute }}>{r.masterProductId}</span>
                  </TD>
                  <TD style={{ color: P.ink2 }}>{r.packagingType ? HD.PACKAGING_TYPE_LABEL[r.packagingType] : '—'}</TD>
                  <TD>
                    {r.tamperEvidentPackaging
                      ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: seal.fg, fontSize: 12.5 }}><Icon name="shield" size={12} stroke={2} /> Skip (tamper-evident)</span>
                      : <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: P.ink2, fontSize: 12.5 }}><Icon name="lock" size={12} stroke={2} /> Seal required</span>}
                  </TD>
                  <TD align="right">
                    <button onClick={() => navigate(`#/admin/catalog/products/${r.masterProductId}`)} style={{ background: 'none', border: 'none', padding: 0, fontSize: 12.5, color: accentInk, cursor: 'pointer', fontFamily: P.fontSans, textDecoration: 'underline', textUnderlineOffset: 2 }}>Edit →</button>
                  </TD>
                </TR>))}
            </tbody>
          </HDTable>
  ...
  function CatalogEditor({ id, navigate }) {
    ...
    return (
      <div style={{ maxWidth: 760, margin: '0 auto', padding: 20 }}>
        <button onClick={() => navigate('#/admin/catalog/products')} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'none', border: 'none', padding: 0, marginBottom: 6, fontSize: 12.5, color: P.inkMute, cursor: 'pointer', fontFamily: P.fontSans }}>
          <Icon name="arrow-left" size={12} stroke={2} /> Back to catalog
        </button>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 30, fontWeight: 600, letterSpacing: '-.02em', color: P.ink }}>{MASTER_LABEL[id] ?? id}</h1>
            <p style={{ margin: '4px 0 0', fontFamily: P.fontMono, fontSize: 11.5, color: P.inkMute }}>{id}</p>
          </div>
          <PBtn variant="accent" onClick={save} disabled={!dirty}>{dirty ? 'Save changes' : 'Saved'}</PBtn>
        </div>
        ...
        <Card padding={16} style={{ marginTop: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            ...
            {tamperEvident
              ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12.5, padding: '2px 8px', borderRadius: 99, background: seal.bg, color: seal.fg, border: `1px solid ${seal.fg}66` }}><Icon name="shield" size={12} stroke={2} /> Auto-skips Sealing</span>
              : <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 12.5, padding: '2px 8px', borderRadius: 99, background: P.surface3, color: P.ink2, border: `1px solid ${P.hairline2}` }}><Icon name="lock" size={12} stroke={2} /> Seal required</span>}
          </div>
          <div style={{ marginTop: 16, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
            <label style={{ display: 'block' }}>
              <span style={{ fontSize: 12.5, color: P.ink2 }}>Packaging type</span>
              <select value={packaging ?? ''} onChange={(e) => { setPackaging(e.target.value); setTamperEvident(TAMPER_DEFAULT.has(e.target.value)); setDirty(true); }}
                style={{ marginTop: 4, width: '100%', background: P.field, border: `1px solid ${P.fieldBorder}`, borderRadius: 8, fontSize: 13.5, padding: '8px 10px', color: P.ink, fontFamily: P.fontSans }}>
            ...
            <div>
              <span style={{ display: 'block', fontSize: 12.5, color: P.ink2 }}>Tamper-evident packaging</span>
              <div style={{ marginTop: 4, display: 'inline-flex', borderRadius: 8, border: `1px solid ${P.hairline3}`, overflow: 'hidden' }}>
                <button onClick={() => { setTamperEvident(true); setDirty(true); }}
                  style={{ padding: '8px 12px', fontSize: 12.5, border: 'none', cursor: 'pointer', fontFamily: P.fontSans, background: tamperEvident ? seal.bg : 'transparent', color: tamperEvident ? seal.fg : P.ink2 }}>Yes — skip seal</button>
                <button onClick={() => { setTamperEvident(false); setDirty(true); }}
                  style={{ padding: '8px 12px', fontSize: 12.5, border: 'none', borderLeft: `1px solid ${P.hairline3}`, cursor: 'pointer', fontFamily: P.fontSans, background: !tamperEvident ? P.surface3 : 'transparent', color: !tamperEvident ? P.ink : P.ink2 }}>No — seal at THC</button>
              </div>
            </div>
          </div>
          <div style={{ marginTop: 16, fontSize: 12.5, color: P.ink2, lineHeight: 1.45 }}>
        ...
        <Card padding={16} style={{ marginTop: 20 }}>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 12.5, color: P.inkDim }}>Live preview of the effective pipeline for this master at THC. Saves apply immediately in-memory.</p>
          <ol style={{ listStyle: 'none', margin: '12px 0 0', padding: 0, display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
            {preview.stages.map((s, i) => (
              <React.Fragment key={s.stageKey}>
                <li style={{ padding: '2px 8px', borderRadius: 8, background: P.surface2, border: `1px solid ${P.hairline2}`, fontSize: 12.5, color: P.ink }}>{s.stageLabel}</li>
                {i < preview.stages.length - 1 && <span style={{ color: P.inkMute }}>→</span>}
              </React.Fragment>))}
            {preview.skippedStages.includes('sealing') && (
              <li style={{ marginLeft: 8, padding: '2px 8px', borderRadius: 8, border: `1px dashed ${seal.fg}66`, color: seal.fg, background: seal.bg, fontSize: 12.5, textDecoration: 'line-through' }}>Sealing skipped (tamper-evident)</li>)}
          </ol>
          {!sealingPresent && <p style={{ margin: '12px 0 0', fontSize: 12.5, color: seal.fg }}>This master skips the shrink-tube station. Floor staff see a "→ Shelf-Ready" arrow on the Labeling card.</p>}
        </Card>
      </div>);
  }
