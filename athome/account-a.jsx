...
function HubScreen({ go }){
  ...
  return (<div>
    {/* hero */}
    <div style={{ position:'relative', padding:'20px 18px 22px', background:P.mode==='dark'?'linear-gradient(160deg,#1c1a12,#111)' : 'linear-gradient(160deg,#151310,#26261f)', overflow:'hidden' }}>
      <div style={{ position:'absolute', top:-70, right:-50, width:220, height:220, borderRadius:'50%', background:`radial-gradient(circle, ${P.accent}30, transparent 70%)` }}/>
      <div style={{ position:'relative', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <span style={{ fontSize:12, fontWeight:800, letterSpacing:'.16em', color:'rgba(255,255,255,.6)', textTransform:'uppercase' }}>Account</span>
      ...
      <div style={{ position:'relative', display:'flex', alignItems:'center', gap:14, marginTop:16 }}>
        <Avatar name={ME.name} size={58} crown/>
        <div>
          <div style={{ fontSize:21, fontWeight:800, color:'#fff', letterSpacing:'-.01em' }}>{ME.name}</div>
          <div style={{ marginTop:5 }}><TierBadge tier={ME.tier}/></div>
        </div>
      </div>
      {/* points progress */}
      <div style={{ position:'relative', marginTop:18, background:'rgba(255,255,255,.07)', border:'1px solid rgba(255,255,255,.1)', borderRadius:15, padding:14 }}>
        <div style={{ display:'flex', alignItems:'baseline', justifyContent:'space-between' }}>
          <span className="mono" style={{ fontSize:22, fontWeight:700, color:P.accent }}>{ME.points.toLocaleString()} <span style={{ fontSize:12, color:'rgba(255,255,255,.6)', fontWeight:600 }}>pts</span></span>
          <span style={{ fontSize:11.5, color:'rgba(255,255,255,.6)' }}>{ME.pointsToNext} to {ME.nextTier}</span>
        </div>
        <div style={{ marginTop:9, height:7, borderRadius:99, background:'rgba(255,255,255,.12)', overflow:'hidden' }}><div style={{ width:(pct*100)+'%', height:'100%', background:P.accent, borderRadius:99 }}/></div>
      </div>
      {/* quick stats */}
      <div style={{ position:'relative', display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, marginTop:10 }}>
        {[['Wallet',money2(ME.wallet)],['Orders',ME.orders],['Member',ME.years]].map((s,i)=>(
          <div key={i} style={{ background:'rgba(255,255,255,.06)', borderRadius:12, padding:'10px 8px', textAlign:'center' }}>
            <div className="mono" style={{ fontSize:15, fontWeight:700, color:'#fff' }}>{s[1]}</div>
            <div style={{ fontSize:10, color:'rgba(255,255,255,.55)', marginTop:2, letterSpacing:'.04em' }}>{s[0]}</div>
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{ padding:'14px 16px 0' }}>
      <Card2 onClick={()=>go('order','A-2041')} pad={0} style={{ overflow:'hidden', border:`1.5px solid ${P.accentBorder}` }}>
        <div style={{ display:'flex', alignItems:'center', gap:12, padding:'13px 15px', background:P.accentSoft }}>
          ...
          <div style={{ flex:1 }}><div style={{ fontSize:13.5, fontWeight:800, color:P.ink }}>Marcus is shopping with you</div><div style={{ fontSize:11.5, color:P.mode==='dark'?'rgba(0,0,0,.6)':'rgba(26,20,0,.7)' }}>@ Home visit · A-2041 · live now</div></div>
          <Icon name="chevron-right" size={20} color={P.accentInk}/>
        </div>
      </Card2>
    </div>
    ...
    <div style={{ padding:'8px 16px 20px' }}>
      <Group title="Orders & shopping">
        <Row icon="receipt" label="Order history" sub="34 orders · track live" onClick={()=>go('orders')}/>
        <Row icon="route" label="Shop @ Home" sub="1 live · book a visit" badge={1} onClick={()=>go('athome')}/>
        <Row icon="star" label="Favorites & reorder" sub={`${FAVES.length} saved`} onClick={()=>go('faves')}/>
      </Group>
      <Group title="Rewards & money">
        <Row icon="target" label="Points & rewards" sub={`${ME.points.toLocaleString()} pts · Gold 2x`} onClick={()=>go('points')}/>
        <Row icon="wallet" label="Wallet & store credit" sub={money2(ME.wallet)+' available'} onClick={()=>go('wallet')}/>
        <Row icon="crown" label="Membership" sub="Gold VIP · perks & tiers" onClick={()=>go('membership')}/>
        <Row icon="gift" label="Refer friends" sub="$25 each · 2 joined" onClick={()=>go('referrals')}/>
      </Group>
      <Group title="Profile">
        <Row icon="user" label="Personal info" sub="Name, phone, email" onClick={()=>go('profile')}/>
      ...
      <div style={{ textAlign:'center', padding:'14px 0 4px', fontSize:11, color:P.inkFaint }} className="mono">Hyperwolf · member since {ME.since}</div>
    </div>
  </div>);
}
