...
function IndexItem({ n, label, meta, live, onClick, last }){ const P=useP(); const [h,setH]=useState(false); return (
  <button onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} onClick={onClick} style={{ width:'100%', display:'flex', alignItems:'center', gap:16, padding:'17px 0', background:'transparent', border:'none', borderBottom:last?'none':`1px solid ${P.hairline}`, cursor:'pointer', textAlign:'left' }}>
    <span className="mono" style={{ fontSize:12, fontWeight:600, color:h?P.accent:P.inkFaint, width:22, transition:'color .13s' }}>{n}</span>
    ...
    {meta && <span className="mono" style={{ fontSize:12, color:P.inkMute }}>{meta}</span>}
...
function HubScreen({ go }){
  ...
  return (<div style={{ padding:'0 22px' }}>
    ...
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'22px 0 0' }}>
      <span className="mono" style={{ fontSize:11, fontWeight:600, letterSpacing:'.2em', textTransform:'uppercase', color:P.inkMute }}>Hyperwolf · Member</span>
    ...
    <div style={{ padding:'34px 0 22px' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, marginTop:16 }}>
        <span style={{ display:'inline-flex', alignItems:'center', gap:6, fontSize:12, fontWeight:700, letterSpacing:'.04em', color:P.ink }}><Icon name="crown" size={14} color={P.accent}/>GOLD VIP</span>
        <span style={{ width:4, height:4, borderRadius:99, background:P.inkFaint }}/>
        <span className="mono" style={{ fontSize:12, color:P.inkMute }}>member {ME.years}</span>
      </div>
    </div>
    ...
    <div style={{ padding:'4px 0 20px', borderTop:`1px solid ${P.hairline2}`, borderBottom:`1px solid ${P.hairline2}` }}>
      <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', paddingTop:20 }}>
        <div><div className="mono" style={{ fontSize:44, fontWeight:700, letterSpacing:'-.02em', lineHeight:.9, color:P.ink }}>{ME.points.toLocaleString()}</div><div className="mono" style={{ fontSize:10.5, letterSpacing:'.16em', color:P.inkMute, marginTop:8 }}>POINTS BALANCE</div></div>
        <div style={{ textAlign:'right' }}><div className="mono" style={{ fontSize:18, fontWeight:700, color:P.ink }}>{money2(ME.wallet)}</div><div className="mono" style={{ fontSize:10.5, letterSpacing:'.16em', color:P.inkMute, marginTop:4 }}>CREDIT</div></div>
      </div>
    ...
    <button onClick={()=>go('order','A-2041')} style={{ width:'100%', display:'flex', alignItems:'center', gap:13, padding:'16px 0', background:'transparent', border:'none', borderBottom:`1px solid ${P.hairline}`, cursor:'pointer', textAlign:'left' }}>
      ...
      <span style={{ flex:1 }}><span style={{ display:'block', fontSize:14, fontWeight:700, color:P.ink }}>Marcus is shopping with you</span><span style={{ display:'block', fontSize:11.5, color:P.inkMute, marginTop:1 }}>@ Home visit in progress · track live</span></span>
    ...
    <div style={{ textAlign:'center', padding:'22px 0 26px' }} className="mono"><span style={{ fontSize:11, color:P.inkFaint, letterSpacing:'.1em' }}>SIGN OUT</span></div>
  </div>);
}
