...
// ══ SCREEN: ACCOUNT HUB ═══════════════════════════════════════════════════
function BentoTile({ icon, label, value, sub, tint, onClick, wide }){ const P=useP(); const [h,setH]=useState(false); return (
  ...
  return (<div>
    ...
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 18px 6px' }}>
      ...
      <div style={{ display:'flex', gap:8 }}>
        <button onClick={toggle} style={{ width:36, height:36, minHeight:44, borderRadius:11, border:`1px solid ${P.hairline2}`, background:P.surface, color:P.inkDim, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}><Icon name={mode==='dark'?'sun':'moon'} size={17}/></button>
        <button onClick={()=>go('settings')} style={{ width:36, height:36, minHeight:44, borderRadius:11, border:`1px solid ${P.hairline2}`, background:P.surface, color:P.inkDim, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }}><Icon name="settings" size={17}/></button>
      </div>
    </div>

    {/* membership card */}
    <div style={{ padding:'8px 16px 4px' }}>
      <div onClick={()=>go('membership')} style={{ position:'relative', overflow:'hidden', borderRadius:20, padding:'18px 20px 20px', cursor:'pointer', background:'linear-gradient(135deg,#2a2410 0%,#13130f 55%,#000 100%)', boxShadow:'0 18px 40px rgba(0,0,0,.35)' }}>
        <div style={{ position:'absolute', top:-50, right:-30, width:180, height:180, borderRadius:'50%', background:`radial-gradient(circle,${P.accent}44,transparent 70%)` }}/>
    ...
    {/* bento grid */}
    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, padding:'12px 16px 2px' }}>
      <BentoTile icon="target" label="Points & rewards" value={ME.points.toLocaleString()} sub="Gold earns 2×" tint={P.accent} onClick={()=>go('points')}/>
      <BentoTile icon="wallet" label="Store credit" value={money2(ME.wallet)} sub="auto-applies" onClick={()=>go('wallet')}/>
      <BentoTile icon="gift" label="Refer friends" value="$50" sub="2 joined · give $25" onClick={()=>go('referrals')}/>
      <BentoTile icon="star" label="Favorites" value={FAVES.length} sub="tap to reorder" onClick={()=>go('faves')}/>
    </div>

    {/* compact list */}
    <div style={{ padding:'12px 16px 20px' }}>
      <Group title="Orders & account">
        <Row icon="receipt" label="Order history" sub="34 orders" onClick={()=>go('orders')}/>
...
window.CustomerAccountAppB = function CustomerAccountAppB(){ return React.createElement(ThemeProvider, null, React.createElement(App)); };
})();
