...
;(function () {
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, borderRadius: P.r12, border: `1px solid ${P.hairline2}`, background: P.surface2, padding: '10px 14px' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 13, fontWeight: 500, color: P.ink }}>{s.label}</span>
                    <HDPill tone="neutral" icon={false} size="sm" label={s.kind} />
                  </div>
                  {s.detail && <div style={{ fontSize: 11, color: P.inkMute, fontFamily: P.fontMono }}>{s.detail}</div>}
                  {s.branchA && <div style={{ marginTop: 4, display: 'flex', gap: 8, fontSize: 11, color: P.inkDim }}><span>↳ {s.branchA}</span><span>↳ {s.branchB}</span></div>}
                </div>
                <span style={{ fontSize: 11, fontFamily: P.fontMono, color: P.inkMute, whiteSpace: 'nowrap' }}>{window.HD.formatNumber(s.entered)} entered</span>
              </div>
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 28, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '-.02em', color: P.ink }}>Flows</h1>
            ...
            return (
              <li key={f.id}>
                <Card padding={16} style={{ cursor: 'pointer', height: '100%' }} onClick={() => navigate(`#/flows/${f.id}`)}>
                  ...
                  <div style={{ marginTop: 12, fontFamily: P.fontMono, fontSize: 11, color: P.inkMute }}>{f.trigger}</div>
                  <div style={{ marginTop: 12, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, paddingTop: 12, borderTop: `1px solid ${P.hairline}` }}>
                    {[['Enrolled', HD.formatNumber(f.enrolled)], ['Converted', HD.formatNumber(f.converted)], ['Revenue', HD.formatCents(f.revenueCents, { showCents: false })]].map(([l, v]) => (
                      <div key={l}>
                        ...
                        <div style={{ fontSize: 14, fontWeight: 600, color: P.ink, fontFamily: P.fontMono }}>{v}</div>
                      </div>))}
                  </div>
                  <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 11, fontFamily: P.fontMono, color: P.inkMute }}>{HD.formatPercent(convRate, 1)} conv</span>
                  </div>
                  <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <span style={{ fontSize: 11, color: P.inkMute }}>v{f.versionNo} · {HD.relativeTime(f.updatedAt)}</span>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
        <header style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            ...
            <h1 style={{ margin: '6px 0 0', fontSize: 28, fontWeight: 700, letterSpacing: '-.02em', color: P.ink }}>{f.name}</h1>
            <p style={{ margin: '4px 0 0', fontSize: 13, color: P.inkMute }}>Version {f.versionNo} · updated {HD.relativeTime(f.updatedAt)}</p>
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 12 }}>
          {[['Total runs', stats.total, 'neutral'], ['Running', stats.running, 'info'], ['Completed', stats.completed, 'ok'], ['Failed / exited', stats.failed + stats.exited, 'warn']].map(([label, value, tone]) => (
            <Card key={label} padding={16}>
              ...
              <div style={{ marginTop: 4, fontSize: 22, fontWeight: 600, fontFamily: P.fontMono, color: tone === 'neutral' ? P.ink : HD.tone(P, tone).fg }}>{HD.formatNumber(value)}</div>
            </Card>))}
        </div>
        ...
        <div className="hd-2col">
          <Card padding={0}>
            <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
              <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Definition</h2>
              <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>{steps.length} nodes · {steps.length - 1} edges · read-only view of the published version</p>
            </header>
          ...
          <aside style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <Card padding={20}>
              ...
              <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 500, color: P.ink }}>{TRIGGER_LABEL(f.trigger)}</span>
              </div>
              <pre style={{ margin: '10px 0 0', overflowX: 'auto', borderRadius: 8, background: P.surface3, padding: 10, fontSize: 11, fontFamily: P.fontMono, color: P.ink2 }}>{JSON.stringify({ eventName: f.trigger, dedupeWindow: '24h', quietHours: true }, null, 2)}</pre>
            </Card>
            <Card padding={20}>
              <MicroLabel>Guardrails</MicroLabel>
              <ul style={{ listStyle: 'none', margin: '8px 0 0', padding: 0, display: 'flex', flexDirection: 'column', gap: 8, fontSize: 12, color: P.ink2 }}>
        ...
        <Card padding={0} style={{ overflow: 'hidden' }}>
          <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
            <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Recent runs</h2>
            <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>Last 25 — full history queryable via the API.</p>
          </header>
          <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
            {runs.map((r) => (
              <li key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 20px', borderBottom: `1px solid ${P.hairline}`, fontSize: 12 }}>
  ...
  window.ScreenFlowTemplates = function ScreenFlowTemplates({ navigate }) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20, padding: 24 }}>
        <header>
          <button onClick={() => navigate('#/flows')} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'none', border: 'none', padding: 0, fontSize: 11, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '.06em', color: P.inkMute, cursor: 'pointer', fontFamily: P.fontSans }}>
            <Icon name="arrow-left" size={12} stroke={2} />Flows
          </button>
          <h1 style={{ margin: '6px 0 0', fontSize: 28, fontWeight: 700, letterSpacing: '-.02em', color: P.ink }}>Start from a template</h1>
          <p style={{ margin: '6px 0 0', maxWidth: 640, fontSize: 13, color: P.inkMute, lineHeight: 1.5 }}>
        ...
        {groups.map(([category, templates]) => (
          <section key={category} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>{category}</h2>
              ...
              {templates.map((t) => (
                <li key={t.id}>
                  <Card padding={16} style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                    ...
                    <p style={{ margin: '8px 0 0', fontSize: 12, color: P.inkDim, lineHeight: 1.45 }}>{t.blurb}</p>
  ...
  window.ScreenFlowNew = function ScreenFlowNew({ navigate }) {
    ...
    const sel = { height: 36, width: '100%', background: P.field, border: `1px solid ${P.fieldBorder}`, borderRadius: P.r10, fontSize: 13, padding: '0 10px', color: P.ink, fontFamily: P.fontSans };
        ...
        <div className="hd-split">
          <section style={{ display: 'flex', flexDirection: 'column', gap: 16, minWidth: 0 }}>
            <div>
              ...
              <h1 style={{ margin: '6px 0 0', fontSize: 24, fontWeight: 600, letterSpacing: '-.02em', color: P.ink }}>New flow</h1>
              <p style={{ margin: '6px 0 0', maxWidth: 560, fontSize: 13, color: P.inkMute, lineHeight: 1.5 }}>
            ...
            <Card padding={20}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Journey</h2>
                <span style={{ fontSize: 11, color: P.inkMute }}>{steps.length} nodes</span>
              </div>
                ...
                <div style={{ marginTop: 12, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {nodes.map((n, i) => (
                    <button key={n.id} onClick={() => setNodes((cur) => cur.filter((_, k) => k !== i))}
                      style={{ display: 'inline-flex', alignItems: 'center', gap: 4, height: 24, padding: '0 8px', borderRadius: 99, background: P.surface3, border: `1px solid ${P.hairline2}`, fontSize: 11, color: P.inkDim, cursor: 'pointer', fontFamily: P.fontSans }}>
                ...
                {PALETTE.map((p) => (
                  <button key={p.kind} onClick={() => add(p.kind)}
                    ...
                    <span style={{ minWidth: 0 }}>
                      <span style={{ display: 'block', fontSize: 12, color: P.ink }}>{p.label}</span>
          ...
          <aside style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <Card padding={20} style={{ background: P.accentSoft, border: `1px solid ${P.accentBorder}` }}>
              ...
              <p style={{ margin: '6px 0 0', fontSize: 11, color: P.inkMute }}>based on the last 4 weeks of {trigger}</p>
            </Card>
            <Card padding={20}>
              <MicroLabel>Guardrails</MicroLabel>
              <label style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: P.ink2, cursor: 'pointer' }}>
              ...
              <p style={{ margin: '12px 0 0', fontSize: 11, color: P.inkMute, lineHeight: 1.5 }}>
            ...
            <Card padding={16}>
              ...
              <ul style={{ listStyle: 'none', margin: '8px 0 0', padding: 0, display: 'flex', flexDirection: 'column', gap: 6, fontSize: 12 }}>
