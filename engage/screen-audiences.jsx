...
;(function () {
        ...
        {options.map((o) => {
          ...
          return (
            <button key={o.value} onClick={() => onToggle(o.value)}
              style={{ height: 24, padding: '0 9px', borderRadius: 99, fontSize: 11, cursor: 'pointer', fontFamily: P.fontSans,
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: 24 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 28, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '-.02em', color: P.ink }}>Audiences</h1>
        ...
        {selected.size > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderRadius: 10, border: `1px solid ${P.accentBorder}`, background: P.accentSoft, padding: '8px 12px', fontSize: 13 }}>
                  ...
                  : visible.map((a) => {
                    ...
                    return (
                      <TR key={a.id} style={{ background: isSel ? P.accentSoft : 'transparent' }}>
                        ...
                        <TD>
                          <button onClick={() => navigate(`#/audiences/${a.id}`)} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', textAlign: 'left', fontFamily: P.fontSans, fontSize: 13, fontWeight: 500, color: P.ink }}>{a.name}</button>
                          <div style={{ fontSize: 11, color: P.inkMute, maxWidth: 320, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.description}</div>
                        </TD>
                        ...
                        <TD><span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 12, color: P.inkDim }}><Icon name="clock" size={11} stroke={2} />{a.refreshCadence}</span></TD>
                        <TD style={{ fontSize: 12, color: P.inkDim }}>{HD.relativeTime(a.lastRefreshedAt)}</TD>
                        <TD align="right">
        ...
        <p style={{ margin: 0, fontSize: 11, color: P.inkMute }}>Showing {visible.length} of {rows.length} audiences{statusF.has('archived') ? '' : ' · archived hidden by default'}</p>
      </div>);
  };
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
        <header style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            ...
            <h1 style={{ margin: '6px 0 0', fontSize: 28, fontWeight: 700, letterSpacing: '-.02em', color: P.ink }}>{a.name}</h1>
            <p style={{ margin: '4px 0 0', maxWidth: 620, fontSize: 13, color: P.inkMute }}>{a.description}</p>
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(230px, 1fr))', gap: 12 }}>
          <Card padding={20} style={{ background: P.accentSoft, border: `1px solid ${P.accentBorder}` }}>
            ...
            <p style={{ margin: '6px 0 0', fontSize: 11, color: P.inkMute }}>as of {HD.formatDateTime(a.lastRefreshedAt)}</p>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
              <div>
                ...
                <div style={{ marginTop: 4, display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 17, fontWeight: 600, color: P.ink }}>{cadence}</span>
                </div>
              </div>
              <select value={cadence} onChange={(e) => { setCadence(e.target.value); window.hdToast?.({ title: 'Cadence updated', description: `Refreshing ${e.target.value}.`, tone: 'ok' }); }}
                style={{ height: 30, background: P.field, border: `1px solid ${P.fieldBorder}`, borderRadius: 8, fontSize: 12, color: P.ink, fontFamily: P.fontSans, padding: '0 8px' }}>
                {['15 min', 'hourly', 'nightly', 'manual'].map((c) => <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <p style={{ margin: '10px 0 0', fontSize: 11, color: P.inkMute }}>Dynamic — re-evaluates against the trait store</p>
          </Card>
          <Card padding={20}>
            ...
            <p style={{ margin: '10px 0 0', fontSize: 11, color: P.inkMute }}>Used by {a.usedByFlows} flows · {a.usedByCampaigns} campaigns</p>
          </Card>
        </div>
        ...
        <Card padding={0}>
          <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
            <div>
              <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Filter</h2>
          ...
          <div style={{ padding: 20 }}>
            <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {chips.map((c) => (
                <li key={c.id} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, borderRadius: 99, border: `1px solid ${P.hairline2}`, background: P.surface3, padding: '5px 12px', fontSize: 13 }}>
                  <span style={{ fontWeight: 500, color: P.ink, fontFamily: P.fontMono, fontSize: 12 }}>{c.path}</span>
                  <HDPill tone="neutral" icon={false} size="sm" label={c.op} />
                  <span style={{ color: P.inkDim }}>{c.display}</span>
                </li>))}
            </ul>
            {a.source === 'ai' && (
              <div style={{ marginTop: 16, borderRadius: 10, border: `1px solid ${P.hairline2}`, background: P.surface2, padding: 12, fontSize: 12, color: P.inkDim }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 12 }}>
          <Card padding={0} style={{ overflow: 'hidden' }}>
            <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
              ...
              <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>Audience lifecycle events from the segmentation engine. Refresh ticks include enter/exit member counts.</p>
            </header>
              ...
              {events.map((e) => (
                <li key={e.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '10px 20px', borderBottom: `1px solid ${P.hairline}`, fontSize: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                    ...
                    <span style={{ color: P.inkDim, fontFamily: P.fontMono, fontSize: 11 }}>{e.meta}</span>
                  </div>
          ...
          <Card padding={0} style={{ overflow: 'hidden' }}>
            <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
              <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Sample customers</h2>
              <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>First 5 matching customer ids · PII redacted at read</p>
            </header>
            <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
              {sample.map((c) => (
                <li key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 20px', borderBottom: `1px solid ${P.hairline}` }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <p style={{ margin: 0, fontFamily: P.fontMono, fontSize: 11, color: P.ink }}>{c.id}</p>
                    <p style={{ margin: 0, fontSize: 11, color: P.inkMute }}>Decrypt context required for email + phone</p>
                  </div>
                </li>))}
            </ul>
          </Card>
        ...
        <Card padding={20}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Build a lookalike</h2>
          </div>
          <p style={{ margin: '4px 0 0', fontSize: 12, color: P.inkMute }}>Expand off this audience's pgvector centroid. Pick how wide to cast — tighter similarity means fewer, better-matched customers.</p>
          <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            {[['0.9 · tight', Math.round(a.size * 0.4)], ['0.8 · balanced', Math.round(a.size * 1.2)], ['0.7 · wide', Math.round(a.size * 3.1)]].map(([label, est]) => (
              <div key={label} style={{ borderRadius: 10, border: `1px solid ${P.hairline2}`, background: P.surface2, padding: '10px 14px' }}>
                ...
                <div style={{ fontSize: 17, fontWeight: 600, color: P.ink, fontFamily: P.fontMono }}>≈{HD.formatNumber(est)}</div>
              </div>))}
  ...
  window.ScreenSuggestedAudiences = function ScreenSuggestedAudiences({ navigate }) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
        <div>
          <button onClick={() => navigate('#/audiences')} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'none', border: 'none', padding: 0, fontSize: 11, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '.06em', color: P.inkMute, cursor: 'pointer', fontFamily: P.fontSans }}>
            <Icon name="arrow-left" size={12} stroke={2} />Audiences
          </button>
          <h1 style={{ margin: '6px 0 0', fontSize: 28, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '-.02em', color: P.ink }}>Suggested audiences</h1>
          <p style={{ margin: '6px 0 0', maxWidth: 640, fontSize: 13, color: P.inkMute, lineHeight: 1.5 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                  <div>
                    ...
                    <div style={{ marginTop: 6, display: 'flex', alignItems: 'baseline', gap: 8 }}>
                      <span style={{ fontSize: 24, fontWeight: 600, color: P.ink, fontFamily: P.fontMono, lineHeight: 1 }}>{HD.formatNumber(s.size)}</span>
                      <span style={{ fontSize: 11, color: P.inkMute }}>estimated members</span>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <MicroLabel align="right">Confidence</MicroLabel>
                    <div style={{ fontSize: 17, fontWeight: 600, fontFamily: P.fontMono, color: s.confidence > 0.8 ? HD.tone(P, 'ok').fg : HD.tone(P, 'warn').fg }}>{HD.formatPercent(s.confidence, 0)}</div>
                  </div>
                </div>
                <p style={{ margin: '12px 0 0', fontSize: 13, color: P.inkDim, lineHeight: 1.45 }}>{s.rationale}</p>
                <div style={{ marginTop: 12, display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {s.signals.map((sig) => (
                    <span key={sig} style={{ display: 'inline-flex', alignItems: 'center', height: 22, padding: '0 8px', borderRadius: 99, background: P.surface3, border: `1px solid ${P.hairline2}`, fontSize: 11, color: P.ink2, fontFamily: P.fontMono }}>{sig}</span>))}
                </div>
  ...
  window.ScreenAudienceCompare = function ScreenAudienceCompare({ navigate }) {
    ...
    const sel = { height: 36, width: '100%', background: P.field, border: `1px solid ${P.fieldBorder}`, borderRadius: P.r10, fontSize: 13, padding: '0 10px', color: P.ink, fontFamily: P.fontSans, marginTop: 6 };
    ...
    const Stat = ({ label, value, sub, primary }) => (
      <div style={{ borderRadius: P.r12, border: `1px solid ${P.hairline2}`, background: P.surface2, padding: 12 }}>
        ...
        <div style={{ marginTop: 4, fontSize: 22, fontWeight: 600, fontFamily: P.fontMono, color: primary ? (P.mode === 'dark' ? P.accent : P.accentBorder) : P.ink }}>{value}</div>
        {sub && <div style={{ marginTop: 2, fontSize: 11, color: P.inkMute, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sub}</div>}
      </div>);
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
        <div>
          ...
          <h1 style={{ margin: '6px 0 0', fontSize: 28, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '-.02em', color: P.ink }}>Compare audiences</h1>
          <p style={{ margin: '6px 0 0', maxWidth: 640, fontSize: 13, color: P.inkMute }}>Intersection, union, Jaccard and containment — computed over member ids, so you can dedupe before a send.</p>
        </div>
        ...
        <Card padding={20}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
            {[['Audience A', aId, setAId, bId], ['Audience B', bId, setBId, aId]].map(([label, val, setter, other]) => (
              <div key={label}>
                <label style={{ fontSize: 12, color: P.ink2 }}>{label}</label>
        ...
        {overlap && (
          <Card padding={20}>
            ...
            <div style={{ marginTop: 12, borderRadius: 10, background: P.surface3, padding: 12, fontSize: 12 }}>
