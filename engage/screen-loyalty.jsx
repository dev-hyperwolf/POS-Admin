...
;(function () {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 28, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '-.02em', color: P.ink }}>Loyalty</h1>
              ...
              <Card padding={20} style={{ cursor: 'pointer', height: '100%' }} onClick={() => navigate(`#/loyalty/${p.id}`)}>
                ...
                <div style={{ marginTop: 12, display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
                  {[['Members', HD.formatNumber(p.members)], ['Points outstanding', HD.formatNumber(p.pointsOutstanding)], ['Liability', HD.formatCents(p.liabilityCents, { showCents: false })], ['Earn rate', p.earnRate]].map(([l, v]) => (
                    <div key={l}>
                      <div style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: '.06em', color: P.inkMute }}>{l}</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
        <header style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <button onClick={() => navigate('#/loyalty')} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'none', border: 'none', padding: 0, fontSize: 11, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '.06em', color: P.inkMute, cursor: 'pointer', fontFamily: P.fontSans }}>
            ...
            <div>
              <h1 style={{ margin: 0, fontSize: 26, fontWeight: 700, letterSpacing: '-.02em', color: P.ink }}>{p.name}</h1>
              <p style={{ margin: '2px 0 0', fontFamily: P.fontMono, fontSize: 11, color: P.inkMute }}>{p.id} · {p.earnRate} · {p.redeemRate} · expires {p.expiryMonths}mo</p>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <HDPill tone={p.status === 'active' ? 'ok' : 'neutral'} icon={false} size="sm" label={p.status} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12 }}>
          {[['Wallets', HD.formatNumber(p.members), 'users', null], ['Active balance', HD.formatNumber(p.pointsOutstanding), 'coins', "= today's points liability"], ['Lifetime earned', HD.formatNumber(Math.round(p.pointsOutstanding * 3.2)), 'trending-up', null], ['Lifetime spent', HD.formatNumber(Math.round(p.pointsOutstanding * 2.2)), 'arrow-down', null]].map(([label, value, icon, hint]) => (
            <Card key={label} padding={16}>
              ...
              <div style={{ marginTop: 4, fontSize: 22, fontWeight: 600, fontFamily: P.fontMono, color: label === 'Lifetime earned' ? HD.tone(P, 'ok').fg : P.ink }}>{value}</div>
        ...
        <Card padding={0} style={{ overflow: 'hidden' }}>
          <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
            <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Points liability · today</h2>
            <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>Daily snapshot from <code style={{ fontFamily: P.fontMono, background: P.surface3, borderRadius: 3, padding: '1px 4px' }}>analytics.liability_rollup</code>; worker writes it nightly.</p>
          </header>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))' }}>
            {[['Starting', liability.startingBalance, null], ['Accrued', liability.accrued, 'ok'], ['Redeemed', -liability.redeemed, null], ['Expired', -liability.expired, 'warn'], ['Adjusted', liability.adjusted, null], ['Ending', liability.endingBalance, 'brand']].map(([label, value, tone], i) => (
              <div key={label} style={{ padding: '14px 20px', borderLeft: i === 0 ? 'none' : `1px solid ${P.hairline}` }}>
                ...
                <div style={{ marginTop: 4, fontSize: 19, fontWeight: 600, fontFamily: P.fontMono, color: tone ? HD.tone(P, tone).fg : P.ink }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 12 }}>
          ...
          <Card padding={0} style={{ overflow: 'hidden' }}>
            <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}`, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Rewards</h2>
              <HDPill tone="neutral" icon={false} size="sm" label={String(p.rewards.length)} />
            </header>
            {p.rewards.length === 0
              ? <p style={{ margin: 0, padding: 20, fontSize: 12, color: P.inkMute }}>No rewards configured yet.</p>
              : <HDTable>
        ...
        <Card padding={0} style={{ overflow: 'hidden' }}>
          <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
            ...
            <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>Loyalty events scoped to this program — credits and debits carry the per-wallet programId tag.</p>
          </header>
          <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
            {events.map(([label, tone, meta, at]) => (
              <li key={label + at} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 20px', borderBottom: `1px solid ${P.hairline}`, fontSize: 12 }}>
                <HDPill tone={tone} icon={false} size="sm" label={label} />
                <span style={{ color: P.inkDim, fontFamily: P.fontMono, fontSize: 11 }}>{meta}</span>
        ...
        <Card padding={0} style={{ overflow: 'hidden' }}>
          <header style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 8, padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
            <div>
              <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Top wallets · by balance</h2>
              <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>Top 10 wallets right now. Click any row to drill into the customer profile.</p>
            </div>
            <span style={{ fontSize: 11, color: P.inkMute }}>of {HD.formatNumber(p.members)}</span>
          </header>
          <HDTable>
  ...
  window.ScreenReferrals = function ScreenReferrals({ navigate }) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 28, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '-.02em', color: P.ink }}>Referrals</h1>
            <p style={{ margin: '6px 0 0', maxWidth: 660, fontSize: 13, color: P.inkMute, lineHeight: 1.5 }}>
        ...
        {fraud > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, borderRadius: 10, border: `1px solid ${HD.tone(P, 'blocked').fg}55`, background: HD.tone(P, 'blocked').bg, padding: 12, fontSize: 12, color: HD.tone(P, 'blocked').fg }}>
        ...
        <Card padding={0} style={{ overflow: 'hidden' }}>
          <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
            <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Funnel</h2>
            <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>Conversion across attribution states. Same row counts as the state filter on each customer.</p>
          </header>
              ...
              return (
                <li key={s.state} style={{ padding: '14px 20px', borderLeft: i === 0 ? 'none' : `1px solid ${P.hairline}` }}>
                  ...
                  <div style={{ marginTop: 4, display: 'flex', alignItems: 'baseline', gap: 6 }}>
                    <span style={{ fontSize: 22, fontWeight: 600, fontFamily: P.fontMono, color: P.ink }}>{HD.formatNumber(s.count)}</span>
                    <span style={{ fontSize: 11, color: P.inkMute, fontFamily: P.fontMono }}>{pct.toFixed(1)}%</span>
                  </div>
        ...
        <Card padding={0} style={{ overflow: 'hidden' }}>
          <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
            <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Top referrers</h2>
            <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>Leaderboard by completed attributions; ties broken by qualified count.</p>
          </header>
          <HDTable>
  ...
  window.ScreenReferralPrograms = function ScreenReferralPrograms({ navigate }) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
        <div>
          <button onClick={() => navigate('#/referrals')} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'none', border: 'none', padding: 0, fontSize: 11, fontWeight: 500, textTransform: 'uppercase', letterSpacing: '.06em', color: P.inkMute, cursor: 'pointer', fontFamily: P.fontSans }}>
            <Icon name="arrow-left" size={12} stroke={2} />Referrals
          </button>
          <h1 style={{ margin: '6px 0 0', fontSize: 28, fontWeight: 700, letterSpacing: '-.02em', color: P.ink }}>Referral programs</h1>
          <p style={{ margin: '6px 0 0', maxWidth: 640, fontSize: 13, color: P.inkMute }}>Both sides of the reward, the qualification rule, and the payout cap. Changing a live program only affects new attributions.</p>
        </div>
        <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 12 }}>
          {programs.map((p) => (
            <li key={p.id}>
              <Card padding={20} style={{ height: '100%' }}>
                ...
                <div style={{ marginTop: 14, paddingTop: 12, borderTop: `1px solid ${P.hairline}`, display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
                  {[['Referrals · 30d', HD.formatNumber(p.referrals30d)], ['Completed', HD.formatNumber(p.completed30d)], ['Revenue', HD.formatCents(p.revenueCents, { showCents: false })], ['Payout', HD.formatCents(p.payoutCents, { showCents: false })]].map(([l, v]) => (
                    <div key={l}>
                      <div style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: '.06em', color: P.inkMute }}>{l}</div>
                      <div style={{ fontSize: 14, fontWeight: 600, fontFamily: P.fontMono, color: P.ink }}>{v}</div>
                    </div>))}
                </div>
                {p.fraudFlagged30d > 0 && <p style={{ margin: '12px 0 0', fontSize: 11, color: HD.tone(P, 'warn').fg }}>{p.fraudFlagged30d} flagged for fraud in the last 30 days</p>}
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
        <div>
          ...
          <h1 style={{ margin: '6px 0 0', fontSize: 28, fontWeight: 700, letterSpacing: '-.02em', color: P.ink }}>Fraud review</h1>
          <p style={{ margin: '6px 0 0', maxWidth: 640, fontSize: 13, color: P.inkMute }}>Attributions the anti-fraud scorer held back. Approving grants both rewards; rejecting writes a suppression and keeps the audit trail.</p>
        </div>
            ...
            <HDTable>
              ...
              <tbody>
                {flagged.map((r) => {
                  ...
                  return (
                    <TR key={r.id}>
                      <TD mono style={{ fontSize: 11 }}>{r.code}</TD>
                      <TD>
                      ...
                      <TD style={{ fontSize: 11, color: P.inkMute }}>{r.riskReasons.length ? r.riskReasons.join(' · ') : 'velocity heuristic'}</TD>
                      <TD align="right" mono>{r.firstOrderCents ? HD.formatCents(r.firstOrderCents) : '—'}</TD>
                      <TD style={{ fontSize: 11, color: P.inkDim }}>{HD.relativeTime(r.attributedAt)}</TD>
                      <TD align="right">
                        {d
  ...
  window.ScreenWallet = function ScreenWallet({ navigate }) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 28, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '-.02em', color: P.ink }}>Wallet passes</h1>
            <p style={{ margin: '6px 0 0', maxWidth: 660, fontSize: 13, color: P.inkMute, lineHeight: 1.5 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, borderRadius: 10, border: `1px solid ${HD.tone(P, 'warn').fg}55`, background: HD.tone(P, 'warn').bg, padding: 12, fontSize: 12, color: HD.tone(P, 'warn').fg }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 12 }}>
          <Card padding={0} style={{ overflow: 'hidden' }}>
            <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
              <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>Newest device adds and removes (last 25).</p>
            </header>
            <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
              {regs.map((r) => (
                <li key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 20px', borderBottom: `1px solid ${P.hairline}`, fontSize: 12 }}>
          ...
          <Card padding={0} style={{ overflow: 'hidden' }}>
            <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
              ...
              <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>Newest pass_state rows; updates fire on every loyalty event.</p>
            </header>
            <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
              {passes.map((p) => (
                <li key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 20px', borderBottom: `1px solid ${P.hairline}`, fontSize: 12 }}>
        ...
        <Card padding={0} style={{ overflow: 'hidden' }}>
          <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
            <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Passes by store</h2>
          </header>
          <HDTable>
                  ...
                  <TR key={s.store}>
                    ...
                    <TD>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ fontSize: 11, fontFamily: P.fontMono, color: P.inkMute }}>{HD.formatPercent(rate, 0)} active</span>
                      </div>
                    </TD>
                  </TR>);
              })}
        ...
        <Card padding={16} style={{ background: P.accentSoft, border: `1px solid ${P.accentBorder}` }}>
          <p style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 600, color: P.mode === 'dark' ? P.accent : P.accentBorder }}>
