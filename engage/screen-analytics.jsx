    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 28, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '-.02em', color: P.ink }}>Analytics</h1>
            ...
            <div style={{ display: 'inline-flex', gap: 2, borderRadius: 8, border: `1px solid ${P.hairline2}`, background: P.surface, padding: 2 }}>
              {[7, 30, 90].map((o) => (
                <button key={o} onClick={() => setWin(o)}
                  style={{ height: 26, padding: '0 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, fontFamily: P.fontMono, cursor: 'pointer', border: 'none',
                    background: win === o ? P.accent : 'transparent', color: win === o ? P.accentInk : P.inkDim }}>{o}d</button>))}
            </div>
        ...
        <Card padding={0}>
          <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
            <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Sends by channel · 14 days</h2>
            <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>Daily message volume per channel from the rollup table.</p>
          </header>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 12 }}>
          <Card padding={0}>
            <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
              <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Attributed revenue by channel</h2>
              <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>Last-touch model · click within 72h of order.</p>
            </header>
            <div style={{ padding: 20 }}>
          ...
          <Card padding={0}>
            <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
              <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Cohort retention · weeks since first order</h2>
              <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>Share of each signup cohort ordering again.</p>
            </header>
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <p style={{ margin: '8px 0 0', fontSize: 11, color: P.inkMute }}>Columns are weeks 0→7 after the first order.</p>
            </div>
          </Card>
        </div>

        <Card padding={20} style={{ background: P.accentSoft, border: `1px solid ${P.accentBorder}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, fontWeight: 600, color: accentInk }}>
            <Icon name="sparkle" size={15} stroke={2} />Derived insights
          </div>
          <ul style={{ listStyle: 'none', margin: '10px 0 0', padding: 0, display: 'flex', flexDirection: 'column', gap: 6 }}>
            {insights.map((i) => (
              <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 6, fontSize: 12, color: P.ink2, lineHeight: 1.5 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }}>
          {[['Deliverability', '#/analytics/deliverability', 'mail'], ['Campaigns', '#/analytics/campaigns', 'megaphone'], ['Attribution', '#/analytics/attribution', 'target'], ['Cohorts', '#/analytics/cohorts', 'grid'], ['RFM matrix', '#/analytics/rfm', 'grid'], ['Geo', '#/analytics/geo', 'map-pin'], ['Staff', '#/analytics/staff', 'users-2'], ['Usage & spend', '#/analytics/usage', 'gauge']].map(([label, href, icon]) => (
            <button key={href} onClick={() => navigate(href)}
              ...
              <span style={{ flex: 1, fontSize: 13, color: P.ink }}>{label}</span>
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        <div>
          ...
          <h1 style={{ margin: '6px 0 0', fontSize: 26, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '-.02em', color: P.ink }}>{title}</h1>
          <p style={{ margin: '6px 0 0', maxWidth: 660, fontSize: 13, color: P.inkMute, lineHeight: 1.5 }}>{blurb}</p>
        </div>
        {right}
      </div>);
  }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
          ...
          {A.perChannel.map((c, i) => {
            ...
            return (
              <Card key={c.channel} padding={16}>
                ...
                <div style={{ marginTop: 4, fontSize: 24, fontWeight: 600, fontFamily: P.fontMono, color: dr > 0.95 ? HD.tone(P, 'ok').fg : HD.tone(P, 'warn').fg }}>{HD.formatPercent(dr, 2)}</div>
                <p style={{ margin: '6px 0 0', fontSize: 11, color: P.inkMute, lineHeight: 1.5 }}>
        ...
        <Card padding={0}>
          <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
            <div>
              <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Delivery rate by channel</h2>
              <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>14-day rollup · delivered ÷ sent.</p>
            </div>
        ...
        <Card padding={0} style={{ overflow: 'hidden' }}>
          <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
            <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Top campaigns · by send volume</h2>
            <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>Deliver, open, click, opt-out rates per campaign over the window.</p>
          </header>
  ...
  window.ScreenCampaignAnalytics = function ScreenCampaignAnalytics({ navigate }) {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
        <SubHeader crumb="Campaigns" title="Campaign analytics" navigate={navigate}
          ...
          right={<div style={{ display: 'inline-flex', gap: 2, borderRadius: 8, border: `1px solid ${P.hairline2}`, background: P.surface, padding: 2 }}>
            {[['revenue', 'Revenue'], ['ctr', 'CTR'], ['sent', 'Volume']].map(([k, l]) => (
              <button key={k} onClick={() => setSortKey(k)} style={{ height: 26, padding: '0 10px', borderRadius: 6, fontSize: 11, fontWeight: 500, cursor: 'pointer', border: 'none', fontFamily: P.fontSans, background: sortKey === k ? P.accent : 'transparent', color: sortKey === k ? P.accentInk : P.inkDim }}>{l}</button>))}
          </div>} />
              ...
              <tbody>
                {rows.map((c) => (
                  <TR key={c.id}>
                    <TD style={{ fontWeight: 500 }}>{c.name}</TD>
                    <TD><HDPill tone="neutral" icon={false} size="sm" label={CHANNEL_LABEL[c.channel] || c.channel} /></TD>
                    <TD style={{ fontSize: 12, color: P.inkDim }}>{c.audience}</TD>
                    <TD align="right" mono>{HD.formatNumber(c.sent)}</TD>
        ...
        <Card padding={0}>
          <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
            <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Revenue per send · by campaign</h2>
          </header>
          <div style={{ padding: 20 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: 24 }}>
        <SubHeader crumb="Attribution" title="Attribution" navigate={navigate}
          ...
          right={<div style={{ display: 'inline-flex', gap: 2, borderRadius: 8, border: `1px solid ${P.hairline2}`, background: P.surface, padding: 2, flexWrap: 'wrap' }}>
            {MODELS.map(([k, l]) => (
              <button key={k} onClick={() => setModel(k)} style={{ height: 26, padding: '0 10px', borderRadius: 6, fontSize: 11, fontWeight: 500, cursor: 'pointer', border: 'none', fontFamily: P.fontSans, background: model === k ? P.accent : 'transparent', color: model === k ? P.accentInk : P.inkDim }}>{l}</button>))}
          </div>} />
        ...
        <Card padding={0}>
          <header style={{ padding: '12px 20px', borderBottom: `1px solid ${P.hairline2}` }}>
            <h2 style={{ margin: 0, fontSize: 14, fontWeight: 600, color: P.ink }}>Revenue share by channel</h2>
            <p style={{ margin: '2px 0 0', fontSize: 11, color: P.inkMute }}>Same underlying touch data, re-weighted by the selected model.</p>
          </header>
          <div style={{ padding: 20 }}>
        ...
        <Card padding={16} style={{ background: P.accentSoft, border: `1px solid ${P.accentBorder}` }}>
          <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: P.mode === 'dark' ? P.accent : P.accentBorder }}>Reading the model swing</p>
