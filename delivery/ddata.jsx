...
// RegionShiftStatus / CalloffStatus — docs/BUILD-AGAINST-THE-SOURCE.md §3: a status
// vocabulary is a contract enum, never a repeated literal. RegionShiftStatus
// ('on'|'off') is region-coverage, a different concern from a driver's own duty
// state (DriverDutyState) even though this file's rows also use the field name
// `status`. Falls back to the literal pair if contracts/index.js has not loaded.
const DHC = window.HWContracts;
const REGION_SHIFT_STATUS = DHC ? DHC.enumValues('RegionShiftStatus') : ['on', 'off'];
const [SHIFT_ON, SHIFT_OFF] = REGION_SHIFT_STATUS;
const CALLOFF_STATUS = DHC ? DHC.enumValues('CalloffStatus') : ['open', 'covered'];
const [CALLOFF_OPEN, CALLOFF_COVERED] = CALLOFF_STATUS;
...
const SUBREGIONS = [
  { id: 'LA-01', county: 'LA', city: 'Pomona',        driver: 'Kofi Mensah',  status: SHIFT_ON, kml: true,  pts: [[236,168],[338,150],[372,214],[330,286],[244,278],[210,220]], override: {} },
  { id: 'LA-02', county: 'LA', city: 'Claremont',     driver: 'Elena Ruiz',   status: SHIFT_ON, kml: true,  pts: [[344,120],[452,132],[470,196],[420,244],[352,224],[330,166]], override: { min: 75 } },
  { id: 'SB-01', county: 'SB', city: 'Fontana',       driver: 'Carlos Diaz',  status: SHIFT_ON, kml: true,  pts: [[458,112],[576,120],[602,190],[548,244],[470,232],[442,168]], override: {} },
  { id: 'SB-02', county: 'SB', city: 'Ontario',       driver: 'Ivy Chen',     status: SHIFT_ON, kml: true,  pts: [[392,224],[498,214],[532,272],[486,330],[404,320],[372,266]], override: { fee: 3 } },
  { id: 'OC-01', county: 'OC', city: 'Yorba Linda',   driver: 'Tara Shah',    status: SHIFT_ON, kml: true,  pts: [[258,318],[372,308],[402,372],[356,432],[276,424],[236,366]], override: {} },
  { id: 'OC-02', county: 'OC', city: 'Brea',          driver: 'Noah Klein',   status: SHIFT_OFF, kml: true, pts: [[196,382],[298,372],[326,432],[286,492],[210,486],[178,428]], override: { close: '9:00p' } },
  { id: 'RC-01', county: 'RC', city: 'Temecula',      driver: 'Theo Reyes',   status: SHIFT_ON, kml: true,  pts: [[528,432],[648,420],[688,486],[644,556],[548,552],[506,486]], override: {} },
  { id: 'RC-02', county: 'RC', city: 'Corona',        driver: 'Sam Okoro',    status: SHIFT_ON, kml: true,  pts: [[470,300],[588,292],[614,356],[566,412],[486,402],[452,344]], override: { fee: 7 } },
  { id: 'RC-03', county: 'RC', city: 'Moreno Valley', driver: 'Dev Anand',    status: SHIFT_ON, kml: false, pts: [[624,252],[792,244],[842,318],[788,388],[652,380],[606,312]], override: {} },
];
...
// window.TDATA.ROSTER rows carry a display `role` string ('driver'|'manager'|'cashier'),
// which is a UI label, not a Classification — HWContracts.ROLE_MAP actually maps the
// string 'driver' to the Role 'associate', so filtering a normalised roster by `role
// === 'driver'` the way this used to would silently empty the schedule the moment a
// roster row carries a real Classification instead. Prefer classification when the
// row has one; only fall back to the legacy `role` label when it does not.
const DRIVERS = (window.TDATA ? window.TDATA.ROSTER : [])
  .filter((p) => ('classification' in p ? p.classification === 'driver' : p.role === 'driver'));
const WEEK = [
...
const CALLOFFS = [
  { day: 'tue', driver: 'Maya Cole',   region: 'RC-02', shift: '10:00a–6:00p', reason: 'Sick', at: '7:42 AM', status: CALLOFF_OPEN,    cover: null },
  { day: 'tue', driver: 'Grace Kim',   region: 'SB-01', shift: '11:00a–7:00p', reason: 'Car trouble', at: '8:15 AM', status: CALLOFF_COVERED, cover: 'Jordan Vu' },
  { day: 'wed', driver: 'Nina Patel',  region: 'OC-01', shift: '9:00a–5:00p',  reason: 'Family emergency', at: 'Yesterday 6:10 PM', status: CALLOFF_OPEN, cover: null },
];
...
window.DDATA = { COUNTIES, COUNTY_BY_ID, SUBREGIONS, effSettings, overriddenKeys, centroid, toPath, bufferPath, WEEK, SCHEDULE_WK, CALLOFFS, DRIVERS,
  ...
  // Contract-backed vocab — exported once here so dapp.jsx reads the same values
  // rather than re-reading HWContracts and risking a second top-level binding.
  SHIFT_ON, SHIFT_OFF, CALLOFF_OPEN, CALLOFF_COVERED };
