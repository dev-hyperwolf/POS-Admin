// ── RFID Direction C — TC22R device frame + handheld chrome ────────────────
// Adapted from mobile/ios-frame.jsx (bezel → absolute status bar → flex content
// → bottom indicator) and mobile/app.jsx's MTopBar / BottomNav, re-shaped for a
// Zebra TC22R: rugged square bezel, physical side triggers, on-screen Android
// nav triad, and a 360×660 logical screen instead of a 402×874 phone.
//
// Ergonomic rules encoded here, and nowhere else:
//   · the ONE action is pinned to the bottom 88px — thumb reach, one-handed
//   · list rows are 64px, action buttons 64px, chips 40px  (HANDOFF: ≥56 on scan)
...
;(function () {
  ...
  // ── The device ────────────────────────────────────────────────────────────
  window.TCDevice = function TCDevice({ children, battery = 0.68, rf = 20, online = true, scanning = false, onTrigger }) {
    const P = useP();
    return (
      <div style={{ position: 'relative', width: 400, flex: '0 0 auto' }}>
        <div style={{
          width: 400, borderRadius: 26, background: P.rail, padding: '18px 20px 22px',
          boxShadow: P.shadowLg, border: `1px solid ${P.hairline2}`, position: 'relative',
        }}>
          ...
          {/* screen */}
          <div style={{
            width: 360, height: 660, borderRadius: 8, overflow: 'hidden', background: P.bg,
            display: 'flex', flexDirection: 'column', position: 'relative',
            boxShadow: `inset 0 0 0 1px ${P.hairline3}`,
          }}>
            <TCStatusBar battery={battery} rf={rf} online={online} scanning={scanning} />
          ...
          {/* physical scan triggers — hardware, deliberately NOT accent-colored:
              accent on this page means the one on-screen primary action. */}
          <button onClick={onTrigger} aria-label="Side scan trigger"
            style={{ position: 'absolute', left: -5, top: 232, width: 8, height: 76, borderRadius: 4, border: 'none', padding: 0,
              background: scanning ? P.good : P.neutral, cursor: 'pointer', transition: 'background .12s' }} />
          <button onClick={onTrigger} aria-label="Side scan trigger"
            style={{ position: 'absolute', right: -5, top: 232, width: 8, height: 76, borderRadius: 4, border: 'none', padding: 0,
              background: scanning ? P.good : P.neutral, cursor: 'pointer', transition: 'background .12s' }} />
        </div>
      </div>);
  };
  ...
  function TCStatusBar({ battery, rf, online, scanning }) {
    ...
    return (
      <div style={{
        flex: '0 0 auto', height: 28, display: 'flex', alignItems: 'center', gap: 8, padding: '0 10px',
        background: P.bg2, borderBottom: `1px solid ${P.hairline}`,
        fontFamily: P.fontMono, fontSize: 10.5, color: P.inkMute, fontVariantNumeric: 'tabular-nums',
      }}>
        <span style={{ color: P.ink2 }}>3:42 PM</span>
        ...
        <span style={{ position: 'relative', width: 18, height: 9, borderRadius: 2, border: `1px solid ${P.inkFaint}` }}>
          <span style={{ position: 'absolute', inset: 1, width: `calc(${Math.round(battery * 100)}% - 2px)`, background: battery < .2 ? P.bad : P.ink2, borderRadius: 1 }} />
        </span>
      </div>);
  }
  ...
  function TCNavTriad() {
    ...
    return (
      <div style={{ flex: '0 0 auto', height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 62, background: P.bg2, borderTop: `1px solid ${P.hairline}` }}>
  ...
  window.HHBar = function HHBar({ title, sub, onBack, right }) {
    const P = useP();
    return (
      <div style={{ flex: '0 0 auto', height: 56, display: 'flex', alignItems: 'center', gap: 4, padding: '0 8px 0 4px', borderBottom: `1px solid ${P.hairline}`, background: P.surface }}>
        {onBack && (
    ...
    return (
      <div style={{ flex: '0 0 auto', padding: '14px 16px 16px', background: c.bg, borderBottom: `2px solid ${c.fg}` }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {icon && <span style={{ display: 'inline-flex', width: 26, height: 26, alignItems: 'center', justifyContent: 'center', borderRadius: 8, background: c.fg + '26', color: c.fg, flex: '0 0 auto' }}><Icon name={icon} size={15} stroke={2.1} /></span>}
  ...
  window.HHRow = function HHRow({ lead, title, sub, value, valueSub, tone, onClick, done, style }) {
    ...
    return (
      <div onClick={onClick} style={{
        ...
        borderRadius: P.r12, cursor: onClick ? 'pointer' : 'default', opacity: done ? .6 : 1, ...style,
      }}>
        {lead}
        ...
        {value != null && (
          <div style={{ textAlign: 'right', flex: '0 0 auto' }}>
            ...
            {valueSub && <div style={{ fontSize: 10.5, color: P.inkMute, fontFamily: P.fontMono, marginTop: 1 }}>{valueSub}</div>}
          </div>)}
      </div>);
  };

  // Square value plate used in 2- and 3-up grids.
  ...
  window.HHAction = function HHAction({ children, sub, icon, variant = 'accent', onClick, disabled }) {
    ...
    const V = {
      ...
      quiet:   { bg: P.surface3, fg: P.ink2, bd: 'transparent' },
    }[variant];
    return (
  ...
  window.HHChips = function HHChips({ value, onChange, options, style }) {
    const P = useP();
    return (
      <div style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 2, ...style }}>
        {options.map((o) => {
  ...
  window.HHScanField = function HHScanField({ scanning, unique, reads, seconds, gate, power }) {
    ...
    return (
      <div style={{
        flex: '0 0 auto', margin: 12, padding: '18px 16px 16px', borderRadius: P.r14,
        background: c.bg, border: `1px solid ${c.fg}55`, position: 'relative', overflow: 'hidden',
      }}>
        {scanning && (
          <div aria-hidden="true" style={{
            position: 'absolute', left: 0, right: 0, top: 0, height: 2, background: c.fg,
            animation: 'rfidsweep 1.5s ease-in-out infinite',
          }} />)}
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          <span style={{ width: 9, height: 9, borderRadius: 99, background: c.fg, boxShadow: scanning ? `0 0 0 4px ${c.fg}33` : 'none' }} />
          ...
          <span style={{ flex: 1 }} />
          <span style={{ fontFamily: P.fontMono, fontSize: 11.5, color: P.inkDim, fontVariantNumeric: 'tabular-nums' }}>{window.RFID.fmtDur(seconds)}</span>
        </div>
  ...
  // Small mono meta line: "label  value" pairs, 2-up.
  window.HHMeta = function HHMeta({ items }) {
    const P = useP();
    return (
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 12px', padding: '10px 12px', background: P.surface3, borderRadius: P.r12 }}>
        {items.map((it, i) => (
  ...
  // Category swatch used as a row lead — the only decorative color, from P.cat.
  window.CatDot = function CatDot({ cat, size = 34, label }) {
    const P = useP();
    const c = P.cat[cat] || P.cat.other;
    return (
      <span style={{
        width: size, height: size, borderRadius: P.r8, flex: '0 0 auto', display: 'flex', alignItems: 'center', justifyContent: 'center',
        background: c + (P.mode === 'dark' ? '2e' : '1f'), color: c, fontFamily: P.fontMono, fontSize: size * 0.36, fontWeight: 700,
      }}>{label != null ? label : cat.slice(0, 2).toUpperCase()}</span>);
  };
})();
