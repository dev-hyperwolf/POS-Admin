  ...
  return (
    <div style={{ padding: '2px 16px 100px', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {isToday && (() => {
        ...
        <div style={{ display: 'flex', gap: 7, marginBottom: 12 }}>{[['all', 'All'], ['cash', 'Cash'], ['card', 'Card'], ['tip', 'Tips']].map(([k, l]) => { const a = ledgerKind === k; return <button key={k} onClick={() => setLedgerKind(k)} style={{ flex: 1, minHeight: 44, padding: '7px 4px', borderRadius: 99, border: `1.5px solid ${a ? P.accentBorder : P.hairline2}`, background: a ? P.accentSoft : 'transparent', color: a ? (P.accentText) : P.ink2, fontSize: 12.5, fontWeight: 700, cursor: 'pointer' }}>{l}</button>; })}</div>
