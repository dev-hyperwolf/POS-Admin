    ...
    <div onClick={onClose} style={{ position: 'absolute', inset: 0, zIndex: 150, background: P.scrim, display: 'flex', alignItems: 'flex-end', animation: 'fade .15s ease' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', background: P.surface, borderRadius: `${P.r24}px ${P.r24}px 0 0`, boxShadow: P.shadowLg, maxHeight: '82%', display: 'flex', flexDirection: 'column', animation: 'sheetUp .24s cubic-bezier(.2,.8,.2,1)', paddingBottom: 24 }}>
        {title !== undefined && <div style={{ display: 'flex', alignItems: 'center', padding: '18px 20px 12px', flex: '0 0 auto' }}>
          <div style={{ fontSize: 21, fontWeight: 700, color: P.ink }}>{title}</div>
          <div style={{ flex: 1 }} />
...
window.OnDutySheet = function OnDutySheet() {
  ...
  return (
    <Dialog actions={<>
      ...
      <div style={{ fontSize: 16, fontWeight: 700, color: P.ink, lineHeight: 1.35 }}>To view today's tasks, please go on duty.</div>
    </Dialog>);
...
window.BreakTimerScreen = function BreakTimerScreen() {
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 34 }}>
        <div style={{ position: 'relative', width: 264, height: 264 }}>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ fontSize: 12.5, color: P.inkMute, fontFamily: P.fontMono, letterSpacing: '.08em', marginTop: 6 }}>REMAINING</div>
          </div>
        </div>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 30, fontWeight: 800, color: P.ink }}>{name}</div>
          <div style={{ fontSize: 16, color: P.inkDim, marginTop: 8 }}>Ends at <b style={{ color: P.ink2, fontFamily: P.fontMono }}>{endTime}</b></div>
        </div>
      </div>);
  };
  ...
  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '0 26px', gap: 30 }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: 13.5, fontWeight: 700, letterSpacing: '.14em', textTransform: 'uppercase', color: almostDone ? P.warn : P.accent, fontFamily: P.fontMono }}>{name}</div>
        ...
        <div style={{ fontSize: 16, color: P.inkDim }}>break time remaining</div>
      </div>
      <div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 10 }}>
          {[['Started', new Date(b.endsAt - total).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })], ['Ends', endTime]].map(([k, v]) => <div key={k} style={{ textAlign: k === 'Ends' ? 'right' : 'left' }}><div style={{ fontSize: 11.5, color: P.inkMute, fontFamily: P.fontMono, letterSpacing: '.06em', textTransform: 'uppercase' }}>{k}</div><div style={{ fontSize: 15, fontWeight: 700, color: P.ink2, fontFamily: P.fontMono, marginTop: 2 }}>{v}</div></div>)}
        </div>
      </div>
    </div>;
  ...
  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', position: 'relative', margin: '0 20px 8px', borderRadius: P.r24, overflow: 'hidden', padding: '30px 26px', justifyContent: 'space-between' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '7px 14px', borderRadius: 99, background: P.mode === 'dark' ? 'rgba(0,0,0,.3)' : 'rgba(255,255,255,.6)' }}><Icon name="clock" size={16} stroke={2.2} color={almostDone ? P.warn : P.ink} /><span style={{ fontSize: 13.5, fontWeight: 800, color: P.ink }}>{name}</span></div>
      </div>
      ...
      {nextStop ? <div style={{ background: P.surface, borderRadius: P.r16, padding: '13px 15px', display: 'flex', alignItems: 'center', gap: 12, position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 11.5, color: P.inkMute, fontFamily: P.fontMono, letterSpacing: '.06em', textTransform: 'uppercase' }}>Next up</div><div style={{ fontSize: 16, fontWeight: 700, color: P.ink, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{nextStop.name} · {nextStop.eta}</div></div>
      </div> : <div />}
    </div>;
...
window.NotifsScreen = function NotifsScreen() {
  ...
  const tone = (t) => ({ accent: [P.accentSoft, P.accentText], info: [P.infoSoft, P.info], warn: [P.warnSoft, P.warn], good: [P.goodSoft, P.good], neutral: [P.neutralSoft, P.ink2] })[t] || [P.surface3, P.ink2];
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px 40px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '70%', textAlign: 'center', padding: '0 30px' }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 800, color: P.ink }}>You're all caught up</div>
        ...
        <div key={g.name} style={{ marginBottom: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '0 2px 9px' }}>
              <span style={{ fontSize: 11.5, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: P.inkMute, fontFamily: P.fontMono }}>{g.name}</span>
              <div style={{ flex: 1, height: 1, background: P.hairline }} />
              <span style={{ fontSize: 11.5, color: P.inkMute, fontFamily: P.fontMono }}>{g.items.length}</span>
            </div>
              ...
              {g.items.map((n) => {const [bg, fg] = tone(n.tone);return (
                <div key={n.id} style={{ display: 'flex', gap: 12, padding: '13px 14px', background: n.unread ? P.surface : P.surface2, border: `1px solid ${P.hairline2}`, borderRadius: P.r14, position: 'relative' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: 13.5, fontWeight: 700, color: P.ink }}>{n.title}</span>{n.unread && <span style={{ width: 7, height: 7, borderRadius: 99, background: P.accent }} />}<div style={{ flex: 1 }} /><span style={{ fontSize: 11.5, color: P.inkMute, fontFamily: P.fontMono, whiteSpace: 'nowrap' }}>{n.time}</span></div>
