      ...
      <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '12px 16px 12px' }}>
        ...
        <>
            ...
            {method === 'cash' && <>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>{[Math.ceil(total), Math.ceil(total / 5) * 5, Math.ceil(total / 10) * 10, Math.ceil(total / 20) * 20].filter((v, i, a) => a.indexOf(v) === i).map((v) => <button key={v} onClick={() => setCash(v.toFixed(2))} style={{ flex: 1, minHeight: 44, padding: '9px 4px', background: P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r10, fontSize: 13.5, fontWeight: 700, color: P.info, cursor: 'pointer', fontFamily: P.fontMono }}>{_m(v)}</button>)}</div>
              <Keypad onPress={pad} />
