    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: P.bg }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 120px' }}>
        ...
        <div style={{ background: P.rail, borderRadius: P.r20, padding: '20px', marginBottom: 14 }}>
          <div style={{ fontSize: 11, letterSpacing: '.1em', textTransform: 'uppercase', color: 'rgba(255,255,255,.6)', fontFamily: P.fontMono }}>Your tip bank · today</div>
          <div style={{ fontSize: 40, fontWeight: 800, color: P.accent, fontFamily: P.fontMono, letterSpacing: '-.01em', marginTop: 4 }}>{_t(total)}</div>
          <div style={{ fontSize: 12, color: 'rgba(255,255,255,.6)', fontFamily: P.fontMono, marginTop: 3 }}>{tips.length} tips · your money, not the company's</div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 13px', background: P.infoSoft, borderRadius: P.r12, marginBottom: 18 }}>
          ...
          <span style={{ fontSize: 12, color: P.info, fontWeight: 600, lineHeight: 1.4 }}>Log tips here so they never get counted against your pouch at reconciliation.</span>
        </div>

        <Eyebrow style={{ marginBottom: 10 }}>Today's tips</Eyebrow>
        {tips.length === 0 ? <div style={{ padding: '30px 0', textAlign: 'center', color: P.inkMute, fontSize: 13 }}>No tips logged yet.</div> :
        <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
          {tips.map((t) => (
            <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 15px', background: P.surface, border: `1px solid ${P.hairline2}`, borderRadius: P.r14 }}>
              <span style={{ width: 38, height: 38, borderRadius: 10, background: P.accentSoft, color: P.mode === 'dark' ? P.accent : '#7A5A00', display: 'flex', alignItems: 'center', justifyContent: 'center', flex: '0 0 auto' }}><Icon name="cash" size={18} stroke={2} /></span>
              <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 14.5, fontWeight: 700, color: P.ink }}>{t.name || 'Cash tip'}</div><div style={{ fontSize: 11, color: P.inkMute, fontFamily: P.fontMono, marginTop: 2 }}>{t.order ? t.order + ' · ' : ''}{t.at}</div></div>
...
window.MakeChangeSheet = function MakeChangeSheet() {
  ...
  return (
    <window.Sheet title="Make change" onClose={() => window.M.closeSheet()} footer={
      ...
      <div style={{ fontSize: 13, color: P.inkDim, lineHeight: 1.5, marginBottom: 16 }}>Break a customer's large bill using your own tip cash — keeps company funds untouched.</div>
      ...
      <div style={{ padding: '15px 16px', borderRadius: P.r14, background: change > 0 ? (enough ? P.goodSoft : P.badSoft) : P.surface2 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}><span style={{ fontSize: 13, fontWeight: 700, color: change > 0 ? (enough ? P.good : P.bad) : P.inkDim }}>Change to give</span><span style={{ fontSize: 22, fontWeight: 800, color: change > 0 ? (enough ? P.good : P.bad) : P.inkFaint, fontFamily: P.fontMono }}>{_t(change)}</span></div>
